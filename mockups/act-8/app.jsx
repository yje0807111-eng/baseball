/* 작전판 8안 — 하단 작전 버튼 모양만 다르다. 치는 회 · 막는 회를 오가며 본다.
   이모지 대신 직접 그린 선 아이콘을 쓰고, 판·테두리는 기존 mt-cut · mt-frame 어휘를 따른다. */
  ...
  <div className="flex items-stretch gap-2.5">
    {list.map(([ic, t, s, on]) => (
      <button key={t} type="button" disabled={!on}
        className="mt-cut mt-frame mt-glass relative flex flex-1 flex-col items-start justify-center overflow-hidden pl-4 pr-3 text-left hover:brightness-125"
        style={{ ...cut('11px'), '--a': tone }}>
        <i className="absolute inset-y-0 left-0 w-[4px]" style={{ background: tone }} />
        <b className="text-[15px] font-extrabold text-white">{t}</b>
...
const A2 = ({ list, tone }) => (
  <div className="flex items-stretch gap-2.5">
    {list.map(([ic, t, s, on]) => (
      <button key={t} type="button" disabled={!on}
        className="mt-cut mt-frame mt-glass flex flex-1 flex-col items-center justify-center gap-0.5 hover:brightness-125"
        style={{ ...cut('11px'), '--a': tone }}>
        <span style={{ color: tone }}>{ICON[ic]}</span>
        <b className="text-[14px] font-bold text-gray-100">{t}</b>
        <small className="text-[12px] font-semibold text-gray-500">{s}</small>
      </button>
    ))}
  </div>
);
...
const A3 = ({ list, tone }) => (
  <div className="flex items-stretch gap-2.5">
    {list.map(([ic, t, s, on]) => (
      <button key={t} type="button" disabled={!on}
        className="mt-cut mt-frame mt-glass flex flex-1 items-center gap-2.5 px-3 text-left hover:brightness-125"
        style={{ ...cut('11px'), '--a': tone }}>
        <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full" style={{ color: tone, background: `${tone}1f`, boxShadow: `inset 0 0 0 1px ${tone}55` }}>{ICON[ic]}</span>
        <span className="min-w-0">
          <b className="block truncate text-[14px] font-bold text-white">{t}</b>
          <small className="text-[11.5px] font-semibold text-gray-400">{s}</small>
        </span>
      </button>
    ))}
  </div>
...
const A4 = ({ list, tone }) => (
  <div className="flex items-stretch gap-2.5">
    {list.map(([ic, t, s, on], i) => (
      <button key={t} type="button" disabled={!on}
        className="mt-cut mt-frame mt-glass relative flex flex-1 flex-col items-center justify-center gap-0.5 hover:brightness-125"
        style={{ ...cut('11px'), '--a': tone }}>
        <em className="absolute left-2 top-1.5 font-display text-[11px] font-extrabold not-italic" style={{ color: `${tone}` }}>{i + 1}</em>
        <span style={{ color: tone }}>{ICON[ic]}</span>
        <b className="text-[14px] font-bold text-gray-100">{t}</b>
        <small className="text-[12px] font-semibold text-gray-500">{s}</small>
      </button>
...
const A5 = ({ list, tone }) => (
  <div className="flex items-stretch gap-2.5">
    {list.map(([ic, t, s, on]) => (
      <button key={t} type="button" disabled={!on}
        className="mt-cut relative flex flex-1 flex-col items-center justify-center gap-0.5 overflow-hidden hover:brightness-125"
        style={{ ...cut('11px'), background: 'rgba(8,12,20,.6)', boxShadow: 'inset 0 0 0 1px rgba(255,255,255,.12)' }}>
        <i className="absolute inset-x-0 top-0 h-[3px]" style={{ background: tone }} />
        <span className="mt-0.5" style={{ color: tone }}>{ICON[ic]}</span>
        <b className="text-[14px] font-bold text-gray-100">{t}</b>
        <small className="text-[12px] font-semibold text-gray-500">{s}</small>
      </button>
    ))}
...
const A6 = ({ list, tone }) => (
  <div className="flex items-center gap-2.5">
    {list.map(([ic, t, s, on]) => (
      <button key={t} type="button" disabled={!on}
        className="flex flex-1 items-center justify-center gap-2 rounded-full py-3 hover:brightness-125"
        style={{ background: 'rgba(8,12,20,.6)', boxShadow: `inset 0 0 0 1px ${tone}55` }}>
        <span style={{ color: tone }}>{ICON[ic]}</span>
        <b className="text-[14px] font-bold text-gray-100">{t}</b>
        <small className="text-[12px] font-semibold text-gray-500">{s}</small>
      </button>
...
const A7 = ({ list, tone }) => (
  <div className="flex items-stretch gap-2.5">
    {list.map(([ic, t, s, on]) => (
      <button key={t} type="button" disabled={!on}
        className="mt-cut group flex flex-1 flex-col items-center justify-center gap-0.5 transition-colors"
        style={{ ...cut('11px'), background: 'transparent', boxShadow: `inset 0 0 0 1.5px ${tone}66`, color: tone }}
        onMouseEnter={(e) => { e.currentTarget.style.background = `${tone}22`; }}
...
const A8 = ({ list, tone }) => (
  <div className="flex items-stretch gap-2.5">
    {list.map(([ic, t, s, on]) => (
      <button key={t} type="button" disabled={!on}
        className="mt-cut mt-frame mt-glass flex flex-1 flex-col items-center justify-center gap-1 hover:brightness-125"
        style={{ ...cut('11px'), '--a': tone }}>
        <span className="flex items-center gap-1.5" style={{ color: tone }}>
          {ICON[ic]}<b className="text-[14px] font-bold text-white">{t}</b>
        </span>
        <span className="mt-cut px-2 py-0.5 text-[11px] font-bold" style={{ ...cut('4px'), background: `${tone}26`, color: tone }}>{s}</span>
      </button>
    ))}
  </div>
);
...
const PLANS = [
  ['1 · 글자만', A1], ['2 · 선 아이콘 ★', A2], ['3 · 가로 · 아이콘 원', A3], ['4 · 숫자 키',

 A4],
  ['5 · 위 색 띠', A5], ['6 · 알약', A6], ['7 · 테두리만', A7], ['8 · 부제 배지', A8],
];
