/* 작전판 배경 8안 — 버튼 모양은 선 아이콘으로 두고 판 바탕만 다르다.
   사진은 Higgsfield 로 뽑은 야간 흙 · 잔디 · 더그아웃 나무. 치는 회 · 막는 회를 오가며 본다. */
...
/* ── 여덟 가지 작전판 — 배경 ─────────────────────────────────── */
const cut = (c) => ({ '--c': c });
const ART = { dirt: 'ui/act/dirt.webp', grass: 'ui/act/grass.webp', wood: 'ui/act/wood.webp' };

/** 사진을 깔고 그 위에 어둡게 덮는 판 — 버튼 다섯이 한 판 안에 들어간다 */
const Board = ({ img, tone, dim = 0.62, children, pos = '50% 50%', extra }) => (
  <div className="mt-cut relative overflow-hidden" style={{ ...cut('12px'), boxShadow: `inset 0 0 0 1px ${tone}3d` }}>
    <i className="absolute inset-0 bg-cover" style={{ backgroundImage: `url(${img})`, backgroundPosition: pos }} />
    <i className="absolute inset-0" style={{ background: `rgba(5,8,15,${dim})` }} />
    {extra}
    <div className="relative flex h-full items-stretch gap-2.5 p-2">{children}</div>
  </div>
);

/** 판 위에 놓이는 버튼 — 속은 비우고 글자만 */
const Ghost = ({ ic, t, s, tone, on }) => (
  <button type="button" disabled={!on}
    className="mt-cut flex flex-1 flex-col items-center justify-center gap-0.5 transition-colors"
    style={{ ...cut('9px'), background: 'rgba(5,8,15,.34)', boxShadow: `inset 0 0 0 1px ${tone}44` }}
    onMouseEnter={(e) => { e.currentTarget.style.background = `${tone}2b`; }}
    onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(5,8,15,.34)'; }}>
    <span style={{ color: tone }}>{ICON[ic]}</span>
    <b className="text-[14px] font-bold text-gray-100">{t}</b>
...
/* 1 · 흙 */
const A1 = ({ list, tone }) => (
  <Board img={ART.dirt} tone={tone}>
    {list.map(([ic, t, s, on]) => <Ghost key={t} ic={ic} t={t} s={s} tone={tone} on={on} />)}
  </Board>
);

/* 2 · 밤 잔디 */
const A2 = ({ list, tone }) => (
  <Board img={ART.grass} tone={tone} dim={0.58}>
    {list.map(([ic, t, s, on]) => <Ghost key={t} ic={ic} t={t} s={s} tone={tone} on={on} />)}
  </Board>
);
...
/* 3 · 더그아웃 나무 */
const A3 = ({ list, tone }) => (
  <Board img={ART.wood} tone={tone} dim={0.55}>
    {list.map(([ic, t, s, on]) => <Ghost key={t} ic={ic} t={t} s={s} tone={tone} on={on} />)}
  </Board>
);
...
/* 4 · 사진 없이 팀 색이 번지는 판 */
const A4 = ({ list, tone }) => (
  <div className="mt-cut relative flex items-stretch gap-2.5 overflow-hidden p-2"
    style={{ ...cut('12px'), background: `linear-gradient(100deg, ${tone}2e, rgba(8,12,20,.72) 42%, rgba(8,12,20,.72) 58%, ${tone}2e)`, boxShadow: `inset 0 0 0 1px ${tone}3d` }}>
    {list.map(([ic, t, s, on]) => <Ghost key={t} ic={ic} t={t} s={s} tone={tone} on={on} />)}
  </div>
);

/* 5 · 흙 판 하나에 구분선만 — 버튼 테두리를 지운다 */
const A5 = ({ list, tone }) => (
  <Board img={ART.dirt} tone={tone} dim={0.66}>
    {list.map(([ic, t, s, on], i) => (
      <button key={t} type="button" disabled={!on}
        className={`relative flex flex-1 flex-col items-center justify-center gap-0.5 ${i ? 'border-l border-white/10' : ''}`}>
        <span style={{ color: tone }}>{ICON[ic]}</span>
        <b className="text-[14px] font-bold text-gray-100">{t}</b>
        <small className="text-[12px] font-semibold text-gray-400">{s}</small>
      </button>
  ...
  <div className="flex items-stretch gap-2.5">
    {list.map(([ic, t, s, on], i) => (
      <button key={t} type="button" disabled={!on}
        ...
        <i className="absolute inset-0 bg-cover" style={{ backgroundImage: `url(${ART.dirt})`, backgroundPosition: `${i * 24}% 50%`, backgroundSize: '420% auto' }} />
...
/* 7 · 위아래 네온 선 */
const A7 = ({ list, tone }) => (
  <Board img={ART.grass} tone={tone} dim={0.7}
    extra={<>
      <i className="absolute inset-x-0 top-0 h-px" style={{ background: `linear-gradient(90deg, transparent, ${tone}, transparent)` }} />
      <i className="absolute inset-x-0 bottom-0 h-px" style={{ background: `linear-gradient(90deg, transparent, ${tone}, transparent)` }} />
    </>}>
    {list.map(([ic, t, s, on]) => <Ghost key={t} ic={ic} t={t} s={s} tone={tone} on={on} />)}
  </Board>
);

/* 8 · 왼쪽에 판 이름을 세우고 그 오른쪽에 버튼 */
const A8 = ({ list, tone }) => (
  <Board img={ART.wood} tone={tone} dim={0.62}>
    <span className="flex shrink-0 flex-col items-center justify-center px-3" style={{ background: `${tone}22`, boxShadow: `inset 0 0 0 1px ${tone}44` }}>
      <b className="font-display text-[13px] font-extrabold tracking-[0.22em]" style={{ color: tone }}>작전</b>
      <small className="mt-0.5 text-[11px] font-semibold text-gray-400">{list === ATT ? '치는 회' : '막는 회'}</small>
    </span>
    {list.map(([ic, t, s, on]) => <Ghost key={t} ic={ic} t={t} s={s} tone={tone} on={on} />)}
  </Board>
);
...
const PLANS = [
  ['1 · 흙', A1], ['2 · 밤 잔디 ★', A2], ['3 · 더그아웃 나무', A3], ['4 · 팀 색 번짐',A4],
  ['5 · 구분선만', A5], ['6 · 칸마다 사진', A6], ['7 · 네온 선', A7], ['8 · 왼쪽에 이름', A8],
];
