/* 작전 버튼 사진 8안 — 버튼마다 그 작전의 한 장면을 깐다. 치는 회 · 막는 회를 오가며 본다.
   사진은 Higgsfield 로 뽑은 야간 실루엣 여덟 장. */
...
/* ── 여덟 가지 — 버튼마다 그 작전의 사진 ───────────────────────── */
const cut = (c) => ({ '--c': c });
/* Higgsfield 로 뽑은 작전별 한 장면 — 전부 야간 · 실루엣 · 얼굴 없음 */
const PIC = {
  steal: 'ui/act/op-steal.webp',   // 2루로 파고드는 슬라이딩
  bunt: 'ui/act/op-bunt.webp',     // 배트를 눕혀 대는 손
  hnr: 'ui/act/op-hnr.webp',       // 주자가 뛰고 타자가 돌린다
  fast: 'ui/act/op-fast.webp',     // 손끝을 떠나는 직구
  slider: 'ui/act/op-slider.webp', // 꺾여 내려가는 변화구
  inside: 'ui/act/op-inside.webp', // 몸쪽으로 파고드는 공
  chase: 'ui/act/op-chase.webp',   // 바깥으로 떨어지는 유인구
  ibb: 'ui/act/op-ibb.webp',       // 팔 벌린 포수, 걸어 나가는 타자
};
  ...
  <div className="flex items-stretch gap-2.5">
    {list.map(([ic, t, s, on]) => (
      <button key={t} type="button" disabled={!on}
        className="mt-cut relative flex flex-1 flex-col items-center justify-center gap-0.5 overflow-hidden"
        style={{ ...cut('11px'), boxShadow: `inset 0 0 0 1px ${tone}44` }}>
        <i className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${PIC[ic]})` }} />
        <i className="absolute inset-0" style={{ background: 'rgba(5,8,15,.66)' }} />
...
const A2 = ({ list, tone }) => (
  <div className="flex items-stretch gap-2.5">
    {list.map(([ic, t, s, on]) => (
      <button key={t} type="button" disabled={!on}
        className="mt-cut relative flex flex-1 items-center overflow-hidden pl-3 text-left"
        style={{ ...cut('11px'), background: 'rgba(8,12,20,.72)', boxShadow: `inset 0 0 0 1px ${tone}44` }}>
        <i className="absolute inset-y-0 right-0 w-1/2 bg-cover bg-center"
          style={{ backgroundImage: `url(${PIC[ic]})`, WebkitMaskImage: 'linear-gradient(90deg,transparent,#000 55%)', maskImage: 'linear-gradient(90deg,transparent,#000 55%)', opacity: 0.55 }} />
        <span className="relative">
          <b className="block text-[14px] font-bold text-white">{t}</b>
          <small className="text-[12px] font-semibold text-gray-300">{s}</small>
        </span>
      </button>
    ))}
  </div>
...
/* 3 · 사진을 아래 띠로 깔고 글자는 위에 */
const A3 = ({ list, tone }) => (
  <div className="flex items-stretch gap-2.5">
    {list.map(([ic, t, s, on]) => (
      <button key={t} type="button" disabled={!on}
        className="mt-cut relative flex flex-1 flex-col items-center justify-start overflow-hidden pt-2"
        style={{ ...cut('11px'), background: 'rgba(8,12,20,.72)', boxShadow: `inset 0 0 0 1px ${tone}44` }}>
        <i className="absolute inset-x-0 bottom-0 h-1/2 bg-cover bg-center"
          style={{ backgroundImage: `url(${PIC[ic]})`, WebkitMaskImage: 'linear-gradient(180deg,transparent,#000)', maskImage: 'linear-gradient(180deg,transparent,#000)', opacity: 0.6 }} />
        <span className="relative flex items-center gap-1.5" style={{ color: tone }}>
          {ICON[ic]}<b className="text-[14px] font-bold text-white">{t}</b>
        </span>
        <small className="relative mt-auto pb-1.5 text-[12px] font-semibold text-gray-200 [text-shadow:0_1px_4px_#000]">{s}</small>
      </button>
    ))}
  </div>
);
...
const A4 = ({ list, tone }) => (
  <div className="flex items-stretch gap-2.5">
    {list.map(([ic, t, s, on]) => (
      <button key={t} type="button" disabled={!on}
        className="mt-cut relative flex flex-1 flex-col items-center justify-center gap-0.5 overflow-hidden"
        style={{ ...cut('11px'), boxShadow: `inset 0 0 0 1px ${tone}55` }}>
        <i className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${PIC[ic]})`, filter: 'grayscale(1)' }} />
        <i className="absolute inset-0" style={{ background: `linear-gradient(180deg, ${tone}3a, rgba(5,8,15,.88))` }} />
        <b className="relative text-[15px] font-extrabold text-white">{t}</b>
        <small className="relative text-[12px] font-semibold text-gray-300">{s}</small>
      </button>
    ))}
  </div>
...
const A5 = ({ list, tone }) => (
  <div className="flex items-stretch gap-2.5">
    {list.map(([ic, t, s, on]) => (
      <button key={t} type="button" disabled={!on}
        className="mt-cut flex flex-1 items-center gap-2.5 overflow-hidden pr-3 text-left"
        style={{ ...cut('11px'), background: 'rgba(8,12,20,.72)', boxShadow: `inset 0 0 0 1px ${tone}44` }}>
        <i className="h-full w-[64px] shrink-0 bg-cover bg-center" style={{ backgroundImage: `url(${PIC[ic]})`, filter: 'brightness(.8)' }} />
        <span className="min-w-0">
          <b className="block truncate text-[14px] font-bold text-white">{t}</b>
...
const A6 = ({ list, tone }) => (
  <div className="flex items-stretch gap-2.5">
    {list.map(([ic, t, s, on]) => (
      <button key={t} type="button" disabled={!on}
        className="mt-cut relative flex flex-1 flex-col items-center justify-center gap-0.5 overflow-hidden"
        style={{ ...cut('11px'), background: 'rgba(8,12,20,.8)', boxShadow: `inset 0 0 0 1px ${tone}44` }}>
        <i className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${PIC[ic]})`, opacity: 0.22 }} />
        <span className="relative" style={{ color: tone }}>{ICON[ic]}</span>
        <b className="relative text-[14px] font-bold text-gray-100">{t}</b>
        <small className="relative text-[12px] font-semibold text-gray-400">{s}</small>
      </button>
    ))}
  </div>
);
...
const A7 = ({ list, tone }) => (
  <div className="flex items-stretch gap-2.5">
    {list.map(([ic, t, s, on]) => (
      <button key={t} type="button" disabled={!on}
        className="mt-cut flex flex-1 items-center gap-2.5 px-3 text-left"
        style={{ ...cut('11px'), background: 'rgba(8,12,20,.72)', boxShadow: `inset 0 0 0 1px ${tone}44` }}>
        <i className="h-10 w-10 shrink-0 rounded-full bg-cover bg-center" style={{ backgroundImage: `url(${PIC[ic]})`, boxShadow: `0 0 0 1.5px ${tone}77` }} />
        <span className="min-w-0">
          <b className="block truncate text-[14px] font-bold text-white">{t}</b>
          <small className="text-[12px] font-semibold text-gray-400">{s}</small>
        </span>
...
/* 8 · 사진이 위에서 아래로 스며든다 — 글자는 아래에 */
const A8 = ({ list, tone }) => (
  <div className="flex items-stretch gap-2.5">
    {list.map(([ic, t, s, on]) => (
      <button key={t} type="button" disabled={!on}
        className="mt-cut relative flex flex-1 flex-col items-center justify-end gap-0 overflow-hidden pb-1.5"
        style={{ ...cut('11px'), background: 'rgba(8,12,20,.72)', boxShadow: `inset 0 0 0 1px ${tone}44` }}>
        <i className="absolute inset-0 bg-cover bg-center"
          style={{ backgroundImage: `url(${PIC[ic]})`, WebkitMaskImage: 'linear-gradient(180deg,#000,transparent 78%)', maskImage: 'linear-gradient(180deg,#000,transparent 78%)', opacity: 0.7 }} />
        <i className="absolute inset-x-0 bottom-0 h-[3px]" style={{ background: tone }} />
        <b className="relative text-[15px] font-extrabold text-white [text-shadow:0_2px_6px_#000]">{t}</b>
        <small className="relative text-[12px] font-semibold text-gray-200 [text-shadow:0_1px_4px_#000]">{s}</small>
      </button>
    ))}
  </div>
);
...
const PLANS = [
  ['1 · 칸 가득', A1], ['2 · 오른쪽 절반', A2], ['3 · 아래 띠', A3], ['4 · 팀 색 덮기',A4],
  ['5 · 왼쪽 네모 ★', A5], ['6 · 아주 옅게', A6], ['7 · 동그랗게', A7], ['8 · 위에서 스밈', A8],
];
