/* 작전 버튼 속 8안 — 사진 없이 그라데이션 · 무늬로만 속을 채운다. 테두리도 함께 짝지었다.
   아이콘과 글자 짜임은 모두 같다. 치는 회 · 막는 회를 오가며 본다. */
...
/* ── 여덟 가지 — 사진 없이 속을 채운다 ─────────────────────────── */
const cut = (c) => ({ '--c': c });
const BASE = 'rgba(8,12,20,.72)'; // 바탕 어두운 유리

/** 버튼 한 장 — fill 로 속을, line 으로 테두리를 정한다 */
const Btns = ({ list, tone, fill, line, extra }) => (
  <div className="flex items-stretch gap-2.5">
    {list.map(([ic, t, s, on]) => (
      <button key={t} type="button" disabled={!on}
        ...
        style={{ ...cut('11px'), background: fill(tone), boxShadow: line(tone) }}>
        {extra && extra(tone)}
        <span className="relative" style={{ color: on ? tone : '#4b5563' }}>{ICON[ic]}</span>
...
/* 1 · 사선 해칭 — 팀 색 실을 얇게 엮는다 */
const A1 = (p) => (
  <Btns {...p}
    fill={(c) => `repeating-linear-gradient(45deg, ${c}0f 0 2px, transparent 2px 9px), ${BASE}`}
    line={(c) => `inset 0 0 0 1px ${c}55`} />
);

/* 2 · 잔 격자 — 전광판 눈금처럼 */
const A2 = (p) => (
  <Btns {...p}
    fill={(c) => `linear-gradient(${c}0d 1px, transparent 1px) 0 0/100% 8px, linear-gradient(90deg, ${c}0d 1px, transparent 1px) 0 0/8px 100%, ${BASE}`}
    line={(c) => `inset 0 0 0 1px ${c}55`} />
);

/* 3 · 위에서 떨어지는 빛 — 누르면 들어갈 듯한 판 */
const A3 = (p) => (
  <Btns {...p}
    fill={() => `linear-gradient(180deg, rgba(255,255,255,.12), rgba(255,255,255,.02) 46%, rgba(0,0,0,.28)), ${BASE}`}
    line={(c) => `inset 0 0 0 1px ${c}4d, inset 0 1px 0 rgba(255,255,255,.28), 0 1px 0 rgba(0,0,0,.5)`} />
);

/* 4 · 모서리에서 번지는 팀 색 */
const A4 = (p) => (
  <Btns {...p}
    fill={(c) => `radial-gradient(120% 90% at 0% 0%, ${c}33, transparent 62%), ${BASE}`}
    line={(c) => `inset 0 0 0 1px ${c}55`} />
);
...
const A5 = (p) => (
  <Btns {...p}
    fill={(c) => `radial-gradient(${c}26 1px, transparent 1.4px) 0 0/7px 7px, ${BASE}`}
    line={(c) => `inset 0 0 0 1px ${c}55`} />
);
...
const A6 = (p) => (
  <Btns {...p}
    fill={() => `repeating-linear-gradient(180deg, rgba(255,255,255,.05) 0 1px, transparent 1px 4px), ${BASE}`}
    line={(c) => `inset 0 0 0 1px ${c}55`} />
);
...
const A7 = (p) => (
  <Btns {...p}
    fill={(c) => `linear-gradient(0deg, ${c}2e, transparent 58%), ${BASE}`}
    line={(c) => `inset 0 0 0 1px ${c}4d`}
    extra={(c) => <i className="absolute inset-x-0 bottom-0 h-[2px]" style={{ background: c }} />} />
);

/* 8 · 이중 테두리 — 안쪽에 한 줄 더, 가운데는 가라앉는다 */
const A8 = (p) => (
  <Btns {...p}
    fill={() => `radial-gradient(90% 120% at 50% 50%, transparent 40%, rgba(0,0,0,.42)), ${BASE}`}
    line={(c) => `inset 0 0 0 1px ${c}66`}
    extra={(c) => <i className="pointer-events-none absolute inset-[4px]" style={{ boxShadow: `inset 0 0 0 1px ${c}26` }} />} />
);
...
const PLANS = [
  ['1 · 사선 해칭', A1], ['2 · 잔 격자', A2], ['3 · 위에서 빛 ★', A3], ['4 · 모서리 번짐',A4],
  ['5 · 점묘', A5], ['6 · 가로 주사선', A6], ['7 · 아래서 차오름', A7], ['8 · 이중 테두리', A8],
];
