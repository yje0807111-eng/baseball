/* 경기 화면 왼쪽 구역 8안 — 배치는 layout-3 의 1안 그대로 두고 왼쪽 칸만 갈아 끼운다.
   판·버튼·초상·수치 묶음은 전부 src/myteam/ui.jsx 의 것을 그대로 쓴다. */
import React, { useState } from 'react';
import { createRoot } from 'react-dom/client';
import '/src/index.css';
import { UiStyle, Panel as UiPanel, Btn, Chip, Portrait, Hero, KV, Stats, SegBar, PlayerTile, SideNav } from '/src/myteam/ui.jsx';
import { KEYFRAMES } from '/src/KboAugmentDraft.jsx';
import { SERIES } from '/src/data/seriesPlayers.js';
import { teamFlag } from '/src/myteam/teamArt.js';
import PlayView from '/src/play/PlayView.jsx';
import { FIELD_BGS } from '/src/play/backgrounds.js';
import { Diamond, Bso, shortTeam, SB_MASK } from '/src/BroadcastGame.jsx';

/* 판 — ui.jsx 의 Panel 에 라벨 여백과 세로 칸만 얹는다 */
const Panel = ({ label, a = '#10b981', c = 14, glass, className = '', style, children }) => (
  <UiPanel a={a} c={c} glass={glass} className={`flex min-h-0 flex-col ${className}`} style={style}>
    {label ? (
      <>
        <p className="mt-lab shrink-0 px-3.5 pb-1 pt-2.5" style={{ '--a': a }}>{label}</p>
        <div className="min-h-0 flex-1 overflow-hidden">{children}</div>
      </>
    ) : children}
  </UiPanel>
);

/* ── 데이터 ──────────────────────────────────────────────────── */
const pick = (id) => SERIES.find((s) => s.id === id) || SERIES[0];
const OFF = pick('2010-lotte');
const DEF = pick('2020-nc');
const bats = (s) => s.players.filter((p) => p.type === 'batter').sort((a, b) => b.overall - a.overall);
const arms = (s) => s.players.filter((p) => p.type === 'pitcher').sort((a, b) => b.overall - a.overall);
const LINEUP = bats(OFF).slice(0, 9);
const NOW = LINEUP[0];
const PEN = arms(OFF).filter((p) => p.position === 'RP').slice(0, 4);
const FOE = arms(DEF)[0];
const DEF9 = (() => {
  const men = bats(DEF);
  const of = men.filter((p) => p.position === 'OF');
  const one = (k) => men.find((p) => p.position === k) || men[0];
  return { P: FOE, C: one('C'), '1B': one('1B'), '2B': one('2B'), '3B': one('3B'), SS: one('SS'), LF: of[0], CF: of[1], RF: of[2] };
})();
const OFF_C = teamFlag(OFF.title)?.color || '#f87171';
const DEF_C = teamFlag(DEF.title)?.color || '#34d399';
const art = (p) => `url(cards/${encodeURIComponent(p.id)}.webp), url(profiles/${encodeURIComponent(p.id)}.webp), url(ui/mt/silhouette-player.webp)`;
const AVG = (i) => `.${240 + i * 7}`;

/* ── 오른쪽·가운데 (1안 그대로, 비교용으로 고정) ────────────────── */
const ACTS = [['도루', '주자 없음'], ['번트', '내 공격 아님'], ['작전 지시', '5가지'], ['선수 교체', '벤치 4'], ['투수 교체', '불펜에서 고르기']];
const Acts = () => (
  <div className="flex h-full gap-3">
    {ACTS.map(([n, s], i) => {
      const on = i === ACTS.length - 1;
      return (
        <Panel key={n} a={on ? '#0b8f68' : '#10b981'} c={12} glass={!on}
          className={`flex flex-1 flex-col items-center justify-center gap-0.5 ${on ? 'bg-[#10b981] text-[#05080f]' : 'text-gray-300'}`}>
          <b className="text-[15px] font-extrabold">{n}</b>
          <span className={`font-display text-[11px] ${on ? 'text-[#05080f]/70' : 'text-gray-500'}`}>{s}</span>
        </Panel>
      );
    })}
  </div>
);
const Head = () => (
  <header className="relative flex items-center gap-5 border-b border-[#10b981]/25 bg-[linear-gradient(180deg,rgba(5,8,15,.94),rgba(5,8,15,.6))] px-6">
    <Btn sm>←</Btn>
    <div className="leading-none">
      <p className="font-display text-[10px] font-semibold uppercase tracking-[0.38em] text-gray-500">Manager Mode</p>
      <h1 className="mt-1 text-xl font-black leading-none text-white">감독 모드</h1>
    </div>
    <Chip a="#f87171">● LIVE</Chip>
    <div className="ml-auto flex gap-2">{['보통', '자동', '스킵'].map((n, i) => <Btn key={n} sm pri={i === 0}>{n}</Btn>)}</div>
  </header>
);
const BG = { ...FIELD_BGS[0], stage: { dy: 0, zoom: 0.98 } };
const Ground = () => (
  <Panel c={16} glass={false} className="relative min-h-0 overflow-hidden bg-[#060c16]">
    <PlayView event={null} bases={[LINEUP[4], null, LINEUP[6]]} defense={DEF9} batter={NOW} offColor={OFF_C} defColor={DEF_C} bg={BG} />
  </Panel>
);
const Right = () => (
  <div className="grid min-h-0 gap-3" style={{ gridTemplateRows: 'auto auto 1fr' }}>
    <Panel label="팀 전략" c={14}>
      <div className="px-3.5 pb-2.5 pt-1">
        {[['전체 전략', '공격적'], ['주루 전략', '적극적'], ['수비 시프트', '기본 수비'], ['불펜 운영', '승리조 우선']].map(([k, v]) => <KV key={k} k={k} v={v} sm />)}
      </div>
    </Panel>
    <Panel label="나의 팀" c={14}>
      <div className="px-3.5 pb-2.5 pt-1">
        {[['선수 컨디션', 77], ['팀 사기', 88], ['체력', 95]].map(([k, v]) => (
          <div key={k} className="py-1.5">
            <div className="flex items-baseline"><span className="text-[12px] text-gray-400">{k}</span>
              <em className="ml-auto font-display text-[12px] font-bold not-italic text-gray-200">{v}</em></div>
            <div className="mt-1"><SegBar pct={v} width={266} ticks={22} /></div>
          </div>
        ))}
      </div>
    </Panel>
    <Panel label="상대 불펜" c={14}>
      <ul className="space-y-1.5 px-3.5 pb-2.5 pt-1">
        {arms(DEF).filter((p) => p.position === 'RP').slice(0, 4).map((p) => (
          <li key={p.id} className="flex items-center gap-2">
            <Portrait player={p} w={22} h={28} color={DEF_C} />
            <b className="truncate text-[13px] font-semibold text-gray-100">{p.name}</b>
            <em className="ml-auto font-display text-[12px] font-bold not-italic text-gray-400">{p.overall}</em>
          </li>
        ))}
      </ul>
    </Panel>
  </div>
);

/* 점수판 — 지금 경기 화면 왼쪽 위에 있던 판을 그대로 옮겨 온다 */
const SIDES = [[OFF.title, 0, OFF_C, false], ['드림팀', 0, DEF_C, true]];
const Scoreboard = () => (
  <div className="mt-cut shrink-0 overflow-hidden backdrop-blur-[3px]"
    style={{ '--c': '12px', background: 'rgba(8,12,20,.55)', boxShadow: 'inset 0 0 0 1px rgba(255,255,255,.14), inset 0 1px 0 rgba(255,255,255,.28)' }}>
    {SIDES.map(([name, runs, c, mine], i) => {
      const flag = teamFlag(name);
      const atBat = !mine;
      return (
        <div key={name} className={`relative flex items-center gap-2.5 overflow-hidden px-3 ${i ? 'border-t border-white/10' : ''}`}
          style={{ height: 52, background: atBat ? `linear-gradient(90deg, ${c}e0, ${c}40 72%, transparent)` : `linear-gradient(90deg, ${c}4d, transparent 60%)` }}>
          {flag && <i className="pointer-events-none absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${flag.src})`, opacity: atBat ? 0.3 : 0.16, WebkitMaskImage: SB_MASK, maskImage: SB_MASK }} />}
          <b className="relative truncate text-[22px] font-extrabold text-white" style={{ textShadow: '0 1px 5px rgba(0,0,0,.6)', opacity: atBat ? 1 : 0.85 }}>{shortTeam(name, mine)}</b>
          {atBat && <span className="relative font-display text-[12px] font-extrabold tracking-[0.18em] text-white/80">AT BAT</span>}
          <b className="relative ml-auto font-display text-[34px] font-extrabold leading-none text-white" style={{ textShadow: '0 1px 6px rgba(0,0,0,.6)', opacity: atBat ? 1 : 0.85 }}>{runs}</b>
        </div>
      );
    })}
    <div className="flex items-stretch border-t border-white/10">
      <span className="grid shrink-0 place-items-center px-3" style={{ background: 'rgba(255,255,255,.07)' }}>
        <b className="font-display text-[21px] font-extrabold leading-none text-white" style={{ textShadow: '0 1px 4px rgba(0,0,0,.75)' }}>4<i className="ml-0.5 not-italic text-[#f87171]">▼</i></b>
      </span>
      <span className="w-px shrink-0 bg-white/12" />
      <span className="grid flex-1 place-items-center py-1"><Bso b={2} s={1} o={1} dot={14} font={13} off="rgba(255,255,255,.45)" lab="text-white/90" /></span>
      <span className="w-px shrink-0 bg-white/12" />
      <span className="grid place-items-center px-1.5"><Diamond bases={[LINEUP[4], null, LINEUP[6]]} size={70} note={38} off="rgba(255,255,255,.45)" ink="rgba(255,255,255,.92)" /></span>
    </div>
  </div>
);

/* ── 왼쪽 8안 ───────────────────────────────────────────────── */
const Row = ({ p, i, por }) => (
  <li className={`mt-row mt-cut team ${i === 0 ? 'on' : ''}`}
    style={{ gridTemplateColumns: por ? `14px ${por}px minmax(0,1fr) auto 24px 34px` : '14px minmax(0,1fr) auto 24px 34px',
      gap: 8, padding: '4px 9px', '--c': '5px', '--a': OFF_C, '--t': OFF_C }}>
    <em className="text-right font-display text-[12px] font-bold not-italic text-gray-500">{i + 1}</em>
    {por && <Portrait player={p} w={por} h={Math.round(por * 1.28)} color={i === 0 ? OFF_C : '#334155'} />}
    <b className="truncate text-[13px] font-semibold text-gray-100">{p.name}</b>
    <span>{i === 0 && <Chip a={OFF_C}>타석</Chip>}</span>
    <span className="font-display text-[11px] text-gray-500">{p.position}</span>
    <em className="text-right font-display text-[12px] font-bold not-italic text-gray-300">{AVG(i)}</em>
  </li>
);
const OrderList = ({ por }) => <ul className="space-y-1 px-2 pb-2">{LINEUP.map((p, i) => <Row key={p.id} p={p} i={i} por={por} />)}</ul>;

const PenList = ({ n = 3 }) => (
  <ul className="space-y-1.5 px-3.5 pb-2.5 pt-1">
    {PEN.slice(0, n).map((p) => (
      <li key={p.id} className="flex items-center gap-2">
        <b className="w-14 truncate text-[13px] font-semibold text-gray-100">{p.name}</b>
        <SegBar pct={p.overall} width={110} ticks={12} />
        <em className="ml-auto font-display text-[12px] font-bold not-italic text-emerald-300">{p.overall}</em>
      </li>
    ))}
  </ul>
);

/* 1 · 라커식 — Hero 초상에 KV 줄 */
const L1 = () => (
  <div className="grid min-h-0 gap-3" style={{ gridTemplateRows: 'auto 1fr' }}>
    <Panel c={14} className="overflow-hidden">
      <Hero img={art(NOW)} ovr={NOW.overall} name={NOW.name} color={OFF_C} h={150} pos="58% 14%" />
      <div className="px-3.5 pb-2.5">
        {[['타율', AVG(0)], ['오늘', '1안타 1볼넷'], ['상대 전적', '4타수 2안타']].map(([k, v]) => <KV key={k} k={k} v={v} sm color={OFF_C} />)}
      </div>
    </Panel>
    <Panel label={`타순 · ${OFF.title}`} c={14}><OrderList /></Panel>
  </div>
);

/* 2 · 카드식 — 드래프트 카드 한 장 */
const L2 = () => (
  <div className="grid min-h-0 gap-3" style={{ gridTemplateRows: 'auto auto 1fr' }}>
    <Panel label="타석" c={14}>
      <div className="flex gap-3 p-3">
        <PlayerTile player={NOW} width={116} height={156} />
        <div className="flex min-w-0 flex-1 flex-col justify-between py-0.5">
          <Stats items={[['타율', AVG(0)], ['홈런', 9]]} />
          <Stats items={[['타점', 55], ['도루', 12]]} />
          <p className="font-display text-[11px] tracking-[0.14em] text-gray-500">오늘 1안타 1볼넷</p>
        </div>
      </div>
    </Panel>
    <Panel label="불펜" c={14}><PenList /></Panel>
    <Panel label="타순" c={14}><OrderList /></Panel>
  </div>
);

/* 3 · 초상 줄 — 타순 아홉 줄에 얼굴을 붙인다 */
const L3 = () => (
  <Panel label={`타순 · ${OFF.title}`} c={14} className="min-h-0">
    <ul className="space-y-1 px-2 pb-2">
      {LINEUP.map((p, i) => (
        <li key={p.id} className={`mt-row mt-cut team ${i === 0 ? 'on' : ''}`}
          style={{ gridTemplateColumns: `12px ${i === 0 ? 40 : 26}px minmax(0,1fr) auto`, gap: 9, padding: i === 0 ? '7px 9px' : '4px 9px', '--c': '6px', '--a': OFF_C, '--t': OFF_C }}>
          <em className="text-right font-display text-[12px] font-bold not-italic text-gray-500">{i + 1}</em>
          <Portrait player={p} w={i === 0 ? 40 : 26} h={i === 0 ? 52 : 34} color={i === 0 ? OFF_C : '#334155'} />
          <div className="min-w-0">
            <b className={`block truncate font-semibold text-gray-100 ${i === 0 ? 'text-[16px]' : 'text-[13px]'}`}>{p.name}</b>
            <span className="font-display text-[10px] tracking-[0.12em] text-gray-500">{p.position} · {AVG(i)}</span>
          </div>
          <em className="font-display text-[14px] font-bold not-italic" style={{ color: i === 0 ? OFF_C : '#94a3b8' }}>{p.overall}</em>
        </li>
      ))}
    </ul>
  </Panel>
);

/* 4 · 탭식 — SideNav 로 타순 · 불펜 · 상대를 갈아 낀다 */
const L4 = () => {
  const [tab, setTab] = useState('타순');
  return (
    <div className="grid min-h-0 gap-3" style={{ gridTemplateRows: 'auto 1fr' }}>
      <Panel c={14} className="overflow-hidden">
        <Hero img={art(NOW)} ovr={NOW.overall} name={NOW.name} color={OFF_C} h={132} pos="58% 14%" />
      </Panel>
      <Panel c={14} className="flex min-h-0 flex-col">
        <div className="px-2 pt-2">
          <SideNav items={['타순', '불펜', '상대']} value={tab} onChange={setTab} a={OFF_C} label="" compact />
        </div>
        <div className="min-h-0 flex-1 overflow-hidden">
          {tab === '타순' && <OrderList />}
          {tab === '불펜' && <PenList n={4} />}
          {tab === '상대' && (
            <div className="px-3.5 pb-2.5 pt-1">
              {[['투수', FOE.name], ['구위', FOE.stats?.stuff ?? '-'], ['제구', FOE.stats?.control ?? '-'], ['체력', '76 / 100']].map(([k, v]) => <KV key={k} k={k} v={v} sm color={DEF_C} />)}
            </div>
          )}
        </div>
      </Panel>
    </div>
  );
};

/* 5 · 기록식 — Stats 묶음과 최근 다섯 타석 */
const L5 = () => (
  <div className="grid min-h-0 gap-3" style={{ gridTemplateRows: 'auto auto 1fr' }}>
    <Panel label="타석" c={14}>
      <div className="flex items-center gap-3 px-3.5 py-2.5">
        <Portrait player={NOW} w={52} h={66} color={OFF_C} />
        <div className="min-w-0 flex-1">
          <b className="block truncate text-[19px] font-extrabold text-white">{NOW.name}</b>
          <span className="font-display text-[11px] tracking-[0.16em] text-gray-500">{NOW.position} · {NOW.hand}</span>
        </div>
        <em className="font-display text-[26px] font-extrabold not-italic" style={{ color: OFF_C }}>{NOW.overall}</em>
      </div>
      <div className="px-3.5 pb-3"><Stats items={[['타율', AVG(0)], ['홈런', 9], ['타점', 55]]} /></div>
    </Panel>
    <Panel label="오늘" c={14}>
      <div className="flex gap-1.5 px-3.5 pb-2.5 pt-1">
        {['안타', '삼진', '볼넷', '뜬공', '타석'].map((v, i) => (
          <span key={v} className="mt-cut flex-1 py-1.5 text-center font-display text-[12px] font-bold"
            style={{ '--c': '6px', background: i === 4 ? `${OFF_C}33` : 'rgba(255,255,255,.05)', color: i === 4 ? '#fff' : '#94a3b8' }}>{v}</span>
        ))}
      </div>
    </Panel>
    <Panel label="타순" c={14}><OrderList /></Panel>
  </div>
);

/* 6 · 능력치 — SegBar 넷 */
const L6 = () => (
  <div className="grid min-h-0 gap-3" style={{ gridTemplateRows: 'auto 1fr' }}>
    <Panel c={14} className="overflow-hidden">
      <Hero img={art(NOW)} ovr={NOW.overall} name={NOW.name} color={OFF_C} h={140} pos="58% 14%" />
      <div className="space-y-2 px-3.5 py-3">
        {[['파워', NOW.stats.power], ['컨택', NOW.stats.contact], ['주력', NOW.stats.speed], ['수비', NOW.stats.defense]].map(([k, v]) => (
          <div key={k} className="flex items-center gap-2">
            <span className="w-8 font-display text-[11px] tracking-[0.12em] text-gray-400">{k}</span>
            <SegBar pct={v} width={188} ticks={20} />
            <em className="ml-auto font-display text-[13px] font-bold not-italic text-gray-200">{v}</em>
          </div>
        ))}
      </div>
    </Panel>
    <Panel label="타순" c={14}><OrderList /></Panel>
  </div>
);

/* 7 · 맞대결 — 타자와 상대 투수를 위아래로 */
const L7 = () => (
  <div className="grid min-h-0 gap-3" style={{ gridTemplateRows: 'auto auto 1fr' }}>
    <Panel label="맞대결" c={14} a={OFF_C}>
      <div className="flex items-center gap-2 px-3 py-3">
        <div className="flex-1 text-center">
          <Portrait player={NOW} w={58} h={74} color={OFF_C} />
          <b className="mt-1.5 block truncate text-[14px] font-bold text-white">{NOW.name}</b>
          <em className="font-display text-[18px] font-extrabold not-italic" style={{ color: OFF_C }}>{NOW.overall}</em>
        </div>
        <span className="font-display text-[13px] font-bold tracking-[0.2em] text-gray-600">VS</span>
        <div className="flex-1 text-center">
          <Portrait player={FOE} w={58} h={74} color={DEF_C} />
          <b className="mt-1.5 block truncate text-[14px] font-bold text-white">{FOE.name}</b>
          <em className="font-display text-[18px] font-extrabold not-italic" style={{ color: DEF_C }}>{FOE.overall}</em>
        </div>
      </div>
      <div className="px-3.5 pb-2.5">{[['오늘', '1안타 1볼넷'], ['상대 전적', '4타수 2안타']].map(([k, v]) => <KV key={k} k={k} v={v} sm />)}</div>
    </Panel>
    <Panel label="불펜" c={14}><PenList /></Panel>
    <Panel label="타순" c={14}><OrderList /></Panel>
  </div>
);

/* 8 · 얇게 — 타순만 촘촘히, 불펜은 점으로 */
const L8 = () => (
  <div className="grid min-h-0 gap-3" style={{ gridTemplateRows: '1fr auto' }}>
    <Panel label={`타순 · ${OFF.title}`} c={14}><OrderList por={20} /></Panel>
    <Panel label="불펜" c={14}>
      <div className="flex flex-wrap gap-1.5 px-3.5 pb-2.5 pt-1.5">
        {PEN.map((p) => (
          <span key={p.id} className="mt-cut flex items-center gap-1.5 bg-white/[0.06] px-2 py-1" style={{ '--c': '5px' }}>
            <i className="h-1.5 w-1.5 rounded-full" style={{ background: p.overall >= 78 ? '#34d399' : '#fbbf24' }} />
            <b className="text-[12px] font-semibold text-gray-200">{p.name}</b>
          </span>
        ))}
      </div>
    </Panel>
  </div>
);

const PLANS = [
  ['1 · 라커식', L1], ['2 · 카드식', L2], ['3 · 초상 줄', L3], ['4 · 탭식', L4],
  ['5 · 기록식', L5], ['6 · 능력치', L6], ['7 · 맞대결', L7], ['8 · 얇게', L8],
];

const Z = Math.min(1, (window.innerWidth - 40) / 1920);

function App() {
  const [i, setI] = useState(0);
  const Left = PLANS[i][1];
  return (
    <div className="min-h-dvh bg-[#06080c] p-4 text-gray-200">
      <UiStyle />
      <style>{KEYFRAMES}</style>
      <div className="mb-3 flex flex-wrap items-center gap-2">
        {PLANS.map(([n], k) => (
          <button key={n} type="button" onClick={() => setI(k)}
            className={`mt-cut px-3 py-1.5 font-display text-[13px] font-bold ${k === i ? 'bg-emerald-400 text-[#05080f]' : 'bg-white/[0.07] text-gray-300'}`} style={{ '--c': '6px' }}>{n}</button>
        ))}
      </div>
      <div style={{ width: 1920 * Z, height: 911 * Z }}>
        <div className="relative select-none overflow-hidden bg-[#05080f] text-gray-200"
          style={{ width: 1920, height: 911, transform: `scale(${Z})`, transformOrigin: 'top left' }}>
          <div className="grid h-full" style={{ gridTemplateRows: '72px 1fr' }}>
            <Head />
            <div className="grid min-h-0 gap-3 p-3" style={{ gridTemplateColumns: '300px 1fr 320px' }}>
              <div className="grid min-h-0 gap-3" style={{ gridTemplateRows: 'auto 1fr' }}>
                <Scoreboard />
                <Left />
              </div>
              <div className="grid min-h-0 gap-3" style={{ gridTemplateRows: '1fr 84px' }}>
                <Ground />
                <Acts />
              </div>
              <Right />
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

createRoot(document.getElementById('root')).render(<App />);
