...
const Order = ({ dark = true, rowH = 29, atH = 44, rows = 9, head = 'label', stripe = false, card = false, teamBar = false, w = 272, round = true }) => {
  const list = LINEUP.slice(0, rows === 9 ? 9 : rows);
  const ink = dark ? '#fff' : INK;
  const f = flagOf(BAT_TEAM.name, BAT_MINE);
  return (
    <div className={`${round ? 'mt-cut' : ''} overflow-hidden ${dark ? 'mt-glass' : ''}`}
      style={{ '--c': '12px', width: w, background: dark ? undefined : PAPER, boxShadow: round ? `inset 0 0 0 1px ${dark ? 'rgba(255,255,255,.14)' : 'rgba(0,0,0,.3)'}` : 'none' }}>
      {head === 'team' && (
        <div className="flex items-center gap-2 px-3 py-1" style={{ background: f.color }}>
          <b className="text-[13px] font-extrabold text-white" style={{ textShadow: '0 1px 4px rgba(0,0,0,.5)' }}>{shortOf(BAT_TEAM.name, BAT_MINE)} 공격</b>
          <span className="ml-auto font-display text-[10.5px] tracking-[0.2em] text-white/75">ORDER</span>
        </div>
      )}
      {head === 'label' && (
        <p className="m-0 px-3 py-1 font-display text-[10.5px] tracking-[0.22em]" style={{ background: dark ? 'rgba(255,255,255,.06)' : 'rgba(255,255,255,.4)', color: dark ? '#9ca3af' : 'rgba(11,18,32,.55)' }}>BATTING ORDER</p>
      )}
      <div className="px-2 py-1">
        {list.map((p, i) => {
          ...
          return (
            <div key={p.n}
              className={`relative flex items-center gap-2 ${card ? 'ui-cut my-[2px] px-2' : `px-1.5 ${i && !stripe ? `border-t ${dark ? 'border-white/[0.07]' : 'border-black/10'}` : ''}`}`}
              style={{ '--c': '5px', height: s === 'at' ? atH : rowH,
                opacity: s === 'wait' ? (dark ? 0.42 : 0.5) : 1,
                background: card ? (s === 'at' ? (dark ? `${ME}26` : 'rgba(52,211,153,.28)') : s === 'on' ? 'rgba(249,115,22,.16)' : dark ? 'rgba(255,255,255,.05)' : 'rgba(255,255,255,.35)')
                  : stripe && i % 2 ? (dark ? 'rgba(255,255,255,.04)' : 'rgba(255,255,255,.28)') : undefined,
                boxShadow: card && s === 'at' ? `inset 0 0 0 1px ${ME}` : undefined }}>
              {teamBar && s !== 'wait' && <i className="absolute inset-y-[3px] left-0 w-[3px] rounded-full" style={{ background: s === 'at' ? (dark ? ME : f.color) : ON }} />}
              <b className={`w-3.5 shrink-0 pl-${teamBar ? 1 : 0} font-display text-[12px]`} style={{ color: dark ? '#6b7280' : 'rgba(11,18,32,.45)' }}>{p.n}</b>
              <b className="truncate" style={{ color: ink, fontSize: s === 'at' ? 17 : 13.5, fontWeight: s === 'at' ? 900 : 700 }}>{p.name}</b>
              {p.on && <span className="inline-block shrink-0" style={{ width: 8, height: 8, background: ON, transform: 'rotate(45deg)', borderRadius: 2 }} />}
              <span className="ml-auto w-12 shrink-0 text-right text-[11.5px]" style={{ color: koColor(p.ko, !dark) }}>{p.ko}</span>
              <b className="w-7 shrink-0 text-right font-display" style={{ fontSize: s === 'at' ? 18 : 13, color: dark ? tone(p.ovr) : INK }}>{p.ovr}</b>
            </div>
          );
        })}
      </div>
...
const Joined = ({ children }) => (
  <div className="mt-cut overflow-hidden" style={{ '--c': '12px', width: 272, boxShadow: 'inset 0 0 0 1px rgba(0,0,0,.35)' }}>{children}</div>
);
const Gap = ({ gap = 8, children }) => <div className="flex flex-col" style={{ gap }}>{children}</div>;

/* ── 16안 ── */
const V = [
  ['1', '한 판 · 밝은 타순', () => (
    <Joined><Score round={false} /><Order dark={false} round={false} head="label" /></Joined>
  )],
  ['2', '한 판 · 어두운 확장부', () => (
    <Joined><Score round={false} /><div style={{ background: 'rgba(8,12,20,.92)' }}><Order round={false} head="label" /></div></Joined>
  )],
  ['3', '살짝 띄움(2)', () => (
    <Gap gap={2}><Score /><Order /></Gap>
  )],
  ['4', '띄움(8) · 팀 머리', () => (
    <Gap gap={8}><Score /><Order head="team" /></Gap>
  )],
  ['5', '한 판 · 공격 팀 머리 띠', () => (
    <Joined><Score round={false} /><Order dark={false} round={false} head="team" /></Joined>
  )],
  ['6', '한 판 · 촘촘(25)', () => (
    <Joined><Score round={false} /><Order dark={false} round={false} rowH={25} atH={40} head="label" /></Joined>
  )],
  ['7', '한 판 · 아주 촘촘(22)', () => (
    <Joined><Score round={false} /><Order dark={false} round={false} rowH={22} atH={38} head="none" /></Joined>
  )],
  ['8', '다음 다섯 명만', () => (
    <Joined><Score round={false} /><Order dark={false} round={false} rows={5} head="label" /></Joined>
  )],
  ['9', '어두운 판 · 팀 색 띠', () => (
    <Gap gap={8}><Score /><Order teamBar head="team" /></Gap>
  )],
  ['10', '밝은 판 · 줄무늬', () => (
    <Joined><Score round={false} /><Order dark={false} round={false} stripe head="label" /></Joined>
  )],
  ['11', '어두운 판 · 줄무늬', () => (
    <Gap gap={2}><Score /><Order stripe head="label" /></Gap>
  )],
  ['12', '칸 카드형', () => (
    <Gap gap={8}><Score /><Order card head="label" /></Gap>
  )],
  ['13', '사이에 팀 색 한 줄', () => (
    <Joined>
      <Score round={false} />
      <div className="px-3 py-1" style={{ background: flagOf(BAT_TEAM.name, BAT_MINE).color }}>
        <b className="text-[12.5px] font-extrabold text-white" style={{ textShadow: '0 1px 4px rgba(0,0,0,.5)' }}>{shortOf(BAT_TEAM.name, BAT_MINE)} 공격</b>
      </div>
      <Order dark={false} round={false} head="none" />
    </Joined>
  )],
  ['14', '밝은 판 · 타석만 팀 색', () => (
    <Joined><Score round={false} /><Order dark={false} round={false} card head="label" /></Joined>
  )],
  ['15', '머리 없이 바로 목록', () => (
    <Gap gap={2}><Score /><Order head="none" /></Gap>
  )],
  ['16', '한 판 · 촘촘 + 팀 머리', () => (
    <Joined><Score round={false} /><Order dark={false} round={false} rowH={24} atH={40} head="team" /></Joined>
  )],
];
  ...
  return (
    <div style={{ width: 1920, background: '#020409', minHeight: '100vh', padding: 20 }}>
      ...
      <div className="mb-3 flex flex-wrap items-center gap-1.5 text-[13px] text-gray-400">
        <b className="mr-1 text-[15px] text-white">왼쪽 합치기 · 16안</b>
        {V.map(([id, name]) => (
