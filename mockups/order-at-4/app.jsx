...
/* 타순 줄에서 "지금 타석" 을 어떻게 더 또렷하게 보일지 네 가지 */
  ...
  <div className="mt-cut overflow-hidden backdrop-blur-[3px]" style={{ '--c': '12px', width: 272, background: 'rgba(8,12,20,.55)', boxShadow: 'inset 0 0 0 1px rgba(255,255,255,.14), inset 0 1px 0 rgba(255,255,255,.28)' }}>
    ...
    <div className="flex items-stretch border-t border-white/10">
      <span className="grid shrink-0 place-items-center px-3" style={{ background: 'rgba(255,255,255,.07)' }}>
        <b className="font-display text-[21px] font-extrabold leading-none text-white">{G.inning}<i className="ml-0.5 not-italic text-[#f87171]">{G.top ? '▲' : '▼'}</i></b>
      </span>
      <span className="w-px shrink-0 bg-white/12" />
      <span className="grid flex-1 place-items-center py-1">
        <div className="grid items-center font-display font-extrabold" style={{ gridTemplateColumns: '15px repeat(3, 14px)', gap: 4, rowGap: 5, fontSize: 13, color: 'rgba(255,255,255,.9)' }}>
          <span className="flex items-center justify-center leading-none" style={{ height: 14 }}>B</span>
          {[0, 1, 2].map((i) => <i key={i} className="rounded-full" style={{ width: 14, height: 14, boxSizing: 'border-box', background: i < G.balls ? '#22c55e' : 'transparent', border: i < G.balls ? 'none' : '1.5px solid rgba(255,255,255,.45)' }} />)}
          <span className="flex items-center justify-center leading-none" style={{ height: 14 }}>S</span>
          {[0, 1].map((i) => <i key={i} className="rounded-full" style={{ width: 14, height: 14, boxSizing: 'border-box', background: i < G.strikes ? '#facc15' : 'transparent', border: i < G.strikes ? 'none' : '1.5px solid rgba(255,255,255,.45)' }} />)}<span />
          <span className="flex items-center justify-center leading-none" style={{ height: 14 }}>O</span>
          {[0, 1].map((i) => <i key={i} className="rounded-full" style={{ width: 14, height: 14, boxSizing: 'border-box', background: i < G.outs ? '#ef4444' : 'transparent', border: i < G.outs ? 'none' : '1.5px solid rgba(255,255,255,.45)' }} />)}<span />
        </div>
      </span>
      <span className="w-px shrink-0 bg-white/12" />
      <span className="grid place-items-center px-1.5">
        <svg viewBox="0 0 100 100" style={{ width: 70, height: 70 }}>
          {[[74, 55], [50, 31], [26, 55]].map(([x, y], i) => (
            <rect key={i} x={x - 13} y={y - 13} width="26" height="26" rx="3" transform={`rotate(45 ${x} ${y})`}
              fill={G.bases[i] ? ON : 'transparent'} stroke={G.bases[i] ? 'none' : 'rgba(255,255,255,.45)'} strokeWidth={G.bases[i] ? 0 : 1.6} />
          ))}
          <text x="50" y="94" textAnchor="middle" fontFamily="'Saira Condensed', sans-serif" fontSize="20" fontWeight="800" fill="rgba(255,255,255,.92)">{G.pitches}</text>
        </svg>
      </span>
    </div>
  </div>
);
/* 타순 판 — mark 로 타석 표시 방법을 바꾼다 */
const Order = ({ mark }) => (
  <div className="mt-cut overflow-hidden backdrop-blur-[3px]" style={{ '--c': '11px', width: 244, background: 'rgba(8,12,20,.55)', boxShadow: 'inset 0 0 0 1px rgba(255,255,255,.14), inset 0 1px 0 rgba(255,255,255,.28)' }}>
    ...
    <div className="px-2 py-1">
      {LINEUP.map((p, i) => {
        ...
        const at = s === 'at';
        return (
          <div key={p.n} className={`relative flex items-center gap-2 px-1.5 ${i ? 'border-t border-white/[0.09]' : ''}`}
            style={{ height: at ? 40 : 27, opacity: s === 'wait' ? 0.55 : 1,
              background: at && (mark === 'bar' || mark === 'glow') ? 'rgba(255,255,255,.1)' : undefined }}>
            {/* 1) 왼쪽 굵은 막대 */}
            {mark === 'bar' && at && <i className="absolute inset-y-0 left-0 w-[4px]" style={{ background: F.color }} />}
            {/* 3) 은은한 빛 + 오른쪽 삼각 */}
            {mark === 'glow' && at && <i className="pointer-events-none absolute inset-0" style={{ background: `linear-gradient(90deg, ${F.color}4d, transparent 65%)` }} />}
            {mark === 'badge' ? (
              <span className="relative grid shrink-0 place-items-center font-display text-[12px] font-extrabold"
                style={{ width: 18, height: 18, borderRadius: 4, background: at ? F.color : 'transparent', color: at ? '#fff' : 'rgba(255,255,255,.4)' }}>{p.n}</span>
            ) : (
              <b className="relative w-3.5 shrink-0 font-display text-[12px] text-white/40">{p.n}</b>
            )}
            {mark === 'bat' && at && <span className="relative shrink-0 text-[13px]">🏏</span>}
            <b className="relative truncate" style={{ color: at && mark === 'bat' ? '#fde047' : '#fff', fontSize: at ? 16 : 13, fontWeight: at ? 900 : 700, textShadow: '0 1px 4px rgba(0,0,0,.6)' }}>{p.name}</b>
            {mark === 'badge' && at && <span className="relative font-display text-[10px] font-extrabold tracking-[0.16em]" style={{ color: F.color }}>타석</span>}
            {p.on && <span className="relative inline-block shrink-0" style={{ width: 8, height: 8, background: ON, transform: 'rotate(45deg)', borderRadius: 2 }} />}
            <span className="relative ml-auto w-12 shrink-0 text-right text-[11.5px]" style={{ color: koDark(p.ko) }}>{p.ko}</span>
            <b className="relative w-7 shrink-0 text-right font-display text-white" style={{ fontSize: at ? 17 : 13 }}>{p.ovr}</b>
            {mark === 'glow' && at && <i className="relative -mr-0.5 shrink-0 not-italic text-[10px]" style={{ color: F.color }}>◀</i>}
          </div>
        );
      })}
    </div>
...
const V = [
  ['1', '왼쪽 굵은 막대', () => <Frame left={<Stack><Board /><Order mark="bar" /></Stack>} />],
  ['2', '번호 배지 + 타석 글자', () => <Frame left={<Stack><Board /><Order mark="badge" /></Stack>} />],
  ['3', '팀 색 번짐 + 화살표', () => <Frame left={<Stack><Board /><Order mark="glow" /></Stack>} />],
  ['4', '배트 아이콘 + 노란 이름', () => <Frame left={<Stack><Board /><Order mark="bat" /></Stack>} />],
];
  ...
  return (
    <div style={{ width: 1920, background: '#020409', minHeight: '100vh', padding: 20 }}>
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2 text-[13px] text-gray-400">
        <b className="text-[15px] text-white">타석 표시 · 4안</b>
        {V.map(([id, name]) => (
