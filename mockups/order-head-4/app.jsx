...
const Rows = () => (
  <div className="px-2 py-1">
    {LINEUP.map((p, i) => {
      const s = state(p);
      return (
        <div key={p.n} className={`flex items-center gap-2 px-1.5 ${i ? 'border-t border-white/[0.09]' : ''}`}
          style={{ height: s === 'at' ? 40 : 27, opacity: s === 'wait' ? 0.55 : 1 }}>
...
const Glassy = ({ children, style, w = 244, c = '11px' }) => (
  <div className="mt-cut overflow-hidden backdrop-blur-[3px]" style={{ '--c': c, width: w, background: 'rgba(8,12,20,.55)', ...style }}>{children}</div>
);
const Stack = ({ children }) => <div className="flex flex-col gap-2">{children}</div>;

/* 1. 팀 색이 왼쪽에서 번지는 머리 + 위쪽 하이라이트 테두리 */
const V1 = () => (
  <Frame left={(
    <Stack>
      <Score />
      <Glassy style={{ boxShadow: 'inset 0 0 0 1px rgba(255,255,255,.14), inset 0 1px 0 rgba(255,255,255,.28)' }}>
        <div className="relative flex items-center gap-2 px-3 py-1.5" style={{ background: `linear-gradient(90deg, ${F.color}cc, ${F.color}33 70%, transparent)` }}>
...
/* 2. 배경 없는 머리 — 팀 색 막대와 아래 실선만 */
const V2 = () => (
  <Frame left={(
    <Stack>
      <Score />
      <Glassy style={{ boxShadow: `inset 0 0 0 1px ${F.color}59` }}>
        <div className="flex items-center gap-2 px-3 py-1.5" style={{ borderBottom: `1px solid ${F.color}80` }}>
          <i className="block h-3.5 w-1 rounded-full" style={{ background: F.color }} />
          <b className="truncate text-[13px] font-extrabold text-white">{NAME} 공격</b>
          <span className="ml-auto font-display text-[10px] tracking-[0.22em] text-white/45">ORDER</span>
        </div>
        <Rows />
      </Glassy>
...
/* 3. 깃발을 옅게 깐 머리 + 이중 테두리 */
const V3 = () => (
  <Frame left={(
    <Stack>
      <Score />
      <Glassy style={{ boxShadow: 'inset 0 0 0 1px rgba(255,255,255,.12), 0 0 0 1px rgba(0,0,0,.5)' }}>
        <div className="relative flex items-center gap-2 overflow-hidden px-3 py-1.5" style={{ background: `${F.color}59` }}>
          ...
          <b className="relative truncate text-[13px] font-extrabold text-white" style={{ textShadow: '0 1px 4px rgba(0,0,0,.6)' }}>{NAME} 공격</b>
          <span className="relative ml-auto font-display text-[10px] tracking-[0.22em] text-white/55">ORDER</span>
        </div>
        <Rows />
      </Glassy>
    </Stack>
...
/* 4. 알약 배지 머리 + 모서리만 팀 색으로 그은 테두리 */
const V4 = () => (
  <Frame left={(
    <Stack>
      <Score />
      <div className="relative">
        <Glassy c="13px" style={{ boxShadow: 'inset 0 0 0 1px rgba(255,255,255,.1)' }}>
          <div className="flex items-center gap-2 px-2.5 py-1.5">
            <span className="rounded-full px-2 py-[3px] text-[11px] font-extrabold text-white" style={{ background: F.color, textShadow: '0 1px 3px rgba(0,0,0,.5)' }}>{NAME}</span>
            <span className="font-display text-[10.5px] tracking-[0.18em] text-white/55">공격 중</span>
            <span className="ml-auto font-display text-[10px] tracking-[0.22em] text-white/35">ORDER</span>
          </div>
          <i className="mx-2.5 block h-px" style={{ background: `linear-gradient(90deg, ${F.color}b3, rgba(255,255,255,.08))` }} />
          <Rows />
        </Glassy>
        {/* 네 귀의 짧은 갈고리 */}
        {[{ left: 0, top: 0 }, { right: 0, top: 0, transform: 'scaleX(-1)' }, { left: 0, bottom: 0, transform: 'scaleY(-1)' }, { right: 0, bottom: 0, transform: 'scale(-1)' }].map((pos, i) => (
          <i key={i} className="pointer-events-none absolute" style={{ width: 12, height: 12, ...pos, borderTop: `2px solid ${F.color}`, borderLeft: `2px solid ${F.color}`, opacity: 0.8 }} />
        ))}
      </div>
    </Stack>
  )} />
...
const V = [['1', '색 번짐 머리', V1], ['2', '실선 머리', V2], ['3', '깃발 머리', V3], ['4', '알약 + 모서리 갈고리', V4]];
  ...
  return (
    <div style={{ width: 1920, background: '#020409', minHeight: '100vh', padding: 20 }}>
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2 text-[13px] text-gray-400">
        <b className="text-[15px] text-white">타순 머리 · 테두리 4안</b>
        {V.map(([id, name]) => (
