...
/* 아래 타순 판은 넷 모두 같다 — 어두운 유리 + 팀 색이 번지는 머리 */
...
const OrderPanel = () => (
  <div className="mt-cut overflow-hidden backdrop-blur-[3px]" style={{ '--c': '11px', width: 244, background: 'rgba(8,12,20,.55)', boxShadow: 'inset 0 0 0 1px rgba(255,255,255,.14), inset 0 1px 0 rgba(255,255,255,.28)' }}>
    <div className="flex items-center gap-2 px-3 py-1.5" style={{ background: `linear-gradient(90deg, ${F.color}cc, ${F.color}33 70%, transparent)` }}>
      <b className="truncate text-[13px] font-extrabold text-white" style={{ textShadow: '0 1px 4px rgba(0,0,0,.6)' }}>{NAME}</b>
      ...
      {LINEUP.map((p, i) => {
        ...
        return (
          <div key={p.n} className={`flex items-center gap-2 px-1.5 ${i ? 'border-t border-white/[0.09]' : ''}`} style={{ height: s === 'at' ? 40 : 27, opacity: s === 'wait' ? 0.55 : 1 }}>
            ...
            <b className="truncate text-white" style={{ fontSize: s === 'at' ? 16 : 13, fontWeight: s === 'at' ? 900 : 700, textShadow: '0 1px 4px rgba(0,0,0,.6)' }}>{p.name}</b>
            ...
            <b className="w-7 shrink-0 text-right font-display text-white" style={{ fontSize: s === 'at' ? 17 : 13, textShadow: '0 1px 4px rgba(0,0,0,.6)' }}>{p.ovr}</b>
          </div>
        );
      })}
    </div>
...
/* 스코어보드 부품 — 안마다 팀 줄과 아래 띠의 결이 다르다 */
const Row = ({ t, mine, fill, h = 52, ink = '#fff' }) => {
  const f = flagOf(t.name, mine);
  const atBat = G.top ? !mine : mine;
  return (
    <div className="relative flex items-center gap-2.5 overflow-hidden px-3" style={{ height: h, background: fill(f) }}>
      <i className="pointer-events-none absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${f.src})`, opacity: 0.3, WebkitMaskImage: MASK, maskImage: MASK }} />
      <b className="relative truncate text-[22px] font-extrabold" style={{ color: ink, textShadow: '0 1px 5px rgba(0,0,0,.6)' }}>{shortOf(t.name, mine)}</b>
      {atBat && <span className="relative font-display text-[12px] font-extrabold tracking-[0.18em]" style={{ color: 'rgba(255,255,255,.8)' }}>AT BAT</span>}
      <b className="relative ml-auto font-display text-[34px] font-extrabold leading-none" style={{ color: ink, textShadow: '0 1px 6px rgba(0,0,0,.6)' }}>{t.runs}</b>
    </div>
  );
};
const Strip = ({ paper = true }) => (
  <div className="flex items-stretch" style={{ background: paper ? PAPER : 'rgba(8,12,20,.5)' }}>
    <span className="grid shrink-0 place-items-center px-3" style={{ background: paper ? 'rgba(255,255,255,.45)' : 'rgba(255,255,255,.07)' }}>
      <b className="font-display text-[21px] font-extrabold leading-none" style={{ color: paper ? INK : '#fff' }}>
        <i className="mr-0.5 not-italic" style={{ color: paper ? '#dc2626' : '#f87171' }}>{G.top ? '▲' : '▼'}</i>{G.inning}
      </b>
    </span>
    <span className="w-px shrink-0" style={{ background: paper ? 'rgba(0,0,0,.2)' : 'rgba(255,255,255,.12)' }} />
    <span className="grid flex-1 place-items-center py-1"><BsoTone paper={paper} /></span>
    <span className="w-px shrink-0" style={{ background: paper ? 'rgba(0,0,0,.2)' : 'rgba(255,255,255,.12)' }} />
    <span className="grid place-items-center px-1.5"><DiamondTone paper={paper} /></span>
  </div>
);
const BsoTone = ({ paper }) => (
  <div className="grid items-center font-display font-extrabold" style={{ gridTemplateColumns: '15px repeat(3, 14px)', gap: 4, rowGap: 5, fontSize: 13, color: paper ? 'rgba(11,18,32,.7)' : 'rgba(255,255,255,.7)' }}>
    <span className="flex items-center justify-center leading-none" style={{ height: 14 }}>B</span>
    {[0, 1, 2].map((i) => <i key={i} className="rounded-full" style={{ width: 14, height: 14, background: i < G.balls ? '#16a34a' : paper ? 'rgba(0,0,0,.16)' : 'rgba(255,255,255,.16)' }} />)}
    <span className="flex items-center justify-center leading-none" style={{ height: 14 }}>S</span>
    {[0, 1].map((i) => <i key={i} className="rounded-full" style={{ width: 14, height: 14, background: i < G.strikes ? '#eab308' : paper ? 'rgba(0,0,0,.16)' : 'rgba(255,255,255,.16)' }} />)}<span />
    <span className="flex items-center justify-center leading-none" style={{ height: 14 }}>O</span>
    {[0, 1].map((i) => <i key={i} className="rounded-full" style={{ width: 14, height: 14, background: i < G.outs ? '#dc2626' : paper ? 'rgba(0,0,0,.16)' : 'rgba(255,255,255,.16)' }} />)}<span />
  </div>
);
const DiamondTone = ({ paper }) => (
  <svg viewBox="0 0 100 100" style={{ width: 70, height: 70 }}>
    {[[74, 55], [50, 31], [26, 55]].map(([x, y], i) => (
      <rect key={i} x={x - 13} y={y - 13} width="26" height="26" rx="3" transform={`rotate(45 ${x} ${y})`} fill={G.bases[i] ? ON : paper ? 'rgba(0,0,0,.16)' : 'rgba(255,255,255,.16)'} />
    ))}
    <text x="50" y="94" textAnchor="middle" fontFamily="'Saira Condensed', sans-serif" fontSize="20" fontWeight="800" fill={paper ? 'rgba(11,18,32,.72)' : 'rgba(255,255,255,.7)'}>{G.pitches}</text>
  </svg>
);
const Board = ({ children, style }) => (
  <div className="mt-cut overflow-hidden backdrop-blur-[3px]" style={{ '--c': '12px', width: 272, ...style }}>{children}</div>
);

/* 1. 팀 색을 가득 채우지 않고 왼쪽에서 번지게 — 아래 띠도 어두운 유리 */
const V1 = () => (
  <Frame left={(
    <Stack>
      <Board style={{ background: 'rgba(8,12,20,.55)', boxShadow: 'inset 0 0 0 1px rgba(255,255,255,.14), inset 0 1px 0 rgba(255,255,255,.28)' }}>
        <Row t={G.away} fill={(f) => `linear-gradient(90deg, ${f.color}cc, ${f.color}33 70%, transparent)`} />
        <i className="block h-px bg-white/10" />
        <Row t={G.home} mine fill={(f) => `linear-gradient(90deg, ${f.color}cc, ${f.color}33 70%, transparent)`} />
        <Strip paper={false} />
      </Board>
      <OrderPanel />
    </Stack>
...
/* 2. 팀 줄만 번지게 두고 아래 띠는 지금처럼 밝은 판 */
const V2 = () => (
  <Frame left={(
    <Stack>
      <Board style={{ background: 'rgba(8,12,20,.55)', boxShadow: 'inset 0 0 0 1px rgba(255,255,255,.14), inset 0 1px 0 rgba(255,255,255,.28)' }}>
        <Row t={G.away} fill={(f) => `linear-gradient(90deg, ${f.color}d9, ${f.color}3d 72%, transparent)`} />
        <i className="block h-px bg-white/10" />
        <Row t={G.home} mine fill={(f) => `linear-gradient(90deg, ${f.color}d9, ${f.color}3d 72%, transparent)`} />
        <Strip />
      </Board>
      <OrderPanel />
    </Stack>
...
/* 3. 팀 색은 왼쪽 굵은 막대만 — 판 전체가 한 톤 */
const V3 = () => (
  <Frame left={(
    <Stack>
      <Board style={{ background: 'rgba(8,12,20,.58)', boxShadow: 'inset 0 0 0 1px rgba(255,255,255,.14), inset 0 1px 0 rgba(255,255,255,.28)' }}>
        {[[G.away, false], [G.home, true]].map(([t, mine], i) => {
          const f = flagOf(t.name, mine);
          const atBat = G.top ? !mine : mine;
          return (
            <div key={t.name} className={`relative flex items-center gap-2.5 overflow-hidden px-3 ${i ? 'border-t border-white/10' : ''}`} style={{ height: 52 }}>
              <i className="absolute inset-y-2 left-0 w-[5px] rounded-full" style={{ background: f.color }} />
              <i className="pointer-events-none absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${f.src})`, opacity: 0.18, WebkitMaskImage: MASK, maskImage: MASK }} />
              <b className="relative ml-1.5 truncate text-[22px] font-extrabold text-white" style={{ textShadow: '0 1px 5px rgba(0,0,0,.6)' }}>{shortOf(t.name, mine)}</b>
              {atBat && <span className="relative font-display text-[12px] font-extrabold tracking-[0.18em] text-white/75">AT BAT</span>}
              <b className="relative ml-auto font-display text-[34px] font-extrabold leading-none text-white">{t.runs}</b>
            </div>
          );
        })}
        <Strip paper={false} />
...
/* 4. 공격 팀 줄만 색이 번지고 수비 팀은 가라앉는다 */
const V4 = () => (
  <Frame left={(
    <Stack>
      <Board style={{ background: 'rgba(8,12,20,.55)', boxShadow: 'inset 0 0 0 1px rgba(255,255,255,.14), inset 0 1px 0 rgba(255,255,255,.28)' }}>
        <Row t={G.away} fill={(f) => (G.top ? `linear-gradient(90deg, ${f.color}e0, ${f.color}40 72%, transparent)` : `linear-gradient(90deg, ${f.color}4d, transparent 60%)`)} />
        <i className="block h-px bg-white/10" />
        <Row t={G.home} mine fill={(f) => (!G.top ? `linear-gradient(90deg, ${f.color}e0, ${f.color}40 72%, transparent)` : `linear-gradient(90deg, ${f.color}4d, transparent 60%)`)} />
        <Strip paper={false} />
      </Board>
      <OrderPanel />
    </Stack>
...
const V = [['1', '번짐 + 어두운 띠', V1], ['2', '번짐 + 밝은 띠', V2], ['3', '왼쪽 막대만', V3], ['4', '공격 팀만 진하게', V4]];
  ...
  return (
    <div style={{ width: 1920, background: '#020409', minHeight: '100vh', padding: 20 }}>
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2 text-[13px] text-gray-400">
        <b className="text-[15px] text-white">스코어보드 유리 · 4안</b>
        {V.map(([id, name]) => (
