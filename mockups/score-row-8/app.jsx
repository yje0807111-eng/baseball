...
/* 위 단은 여덟 안 모두 같다 — 회 기둥 + 팀 두 줄 */
const Top = ({ w = 218 }) => (
  <div className="flex items-stretch"><InnCol /><Teams w={w} /></div>
);
const PitchCell = ({ w = 46 }) => (
  <span className="grid place-items-center" style={{ width: w }}>
    <span className="text-center leading-tight">
      <b className="block font-display text-[17px] font-extrabold" style={{ color: INK }}>{G.pitches}</b>
      <small className="font-display text-[9.5px] tracking-[0.12em]" style={{ color: 'rgba(11,18,32,.5)' }}>PITCH</small>
    </span>
  </span>
);

/* 1. 기준 — 카운트 넓게, 주루 가운데, 투구 수 오른쪽 */
const V1 = () => (
  <Frame board={(
    <Two w={258}>
      <Top />
      <div className="flex items-stretch border-t border-black/20">
        ...
        <Sep /><span className="grid place-items-center px-2"><Diamond size={62} /></span>
        <Sep /><PitchCell />
      </div>
    </Two>
  )} />
...
const V2 = () => (
  <Frame board={(
    <Two w={258}>
      <Top />
      <div className="flex items-stretch border-t border-black/20">
        <span className="flex flex-1 items-center pl-3"><Bso dot={13} /></span>
        <Sep /><span className="grid place-items-center px-2"><Diamond size={62} /></span>
        <Sep /><PitchCell />
      </div>
    </Two>
...
const V4 = () => (
  <Frame board={(
    <Two w={258}>
      <Top />
      <div className="grid items-stretch border-t border-black/20" style={{ gridTemplateColumns: 'repeat(3,1fr)' }}>
        <span className="grid place-items-center py-1"><Bso dot={12} font={12} /></span>
        <span className="grid place-items-center border-x border-black/20 py-1"><Diamond size={60} /></span>
        <PitchCell w="100%" />
      </div>
    </Two>
  )} />
...
const V5 = () => (
  <Frame board={(
    <Two w={258}>
      <Top />
      <div className="flex items-stretch border-t border-black/20">
        <span className="grid place-items-center px-2.5" style={{ width: 78 }}><Bso dot={11} font={12} /></span>
        <Sep /><span className="grid flex-1 place-items-center py-1"><Diamond size={74} /></span>
        <Sep /><PitchCell w={44} />
      </div>
    </Two>
  )} />
...
/* 7. 카운트를 가로 한 줄로 깔고 그 오른쪽에 주루 · 투구 */
const V7 = () => (
  <Frame board={(
    <Two w={258}>
      <Top />
      <div className="flex items-stretch border-t border-black/20">
        <span className="flex flex-1 flex-col justify-center gap-1.5 px-3">
          ...
          <span className="flex items-center justify-between font-display text-[12px] font-extrabold" style={{ color: 'rgba(11,18,32,.6)' }}><span>투구</span><b style={{ color: INK }}>{G.pitches}</b></span>
        </span>
        <Sep /><span className="grid place-items-center px-2"><Diamond size={64} /></span>
      </div>
    </Two>
  )} />
);

/* 8. 주루를 왼쪽 끝으로, 카운트와 투구 수는 오른쪽에 나란히 */
const V8 = () => (
  <Frame board={(
    <Two w={258}>
      <Top />
      <div className="flex items-stretch border-t border-black/20">
        <span className="grid place-items-center px-2"><Diamond size={62} /></span>
        <Sep /><span className="grid flex-1 place-items-center py-1"><Bso dot={13} /></span>
        <Sep /><PitchCell />
      </div>
    </Two>
  )} />
...
const V = [['1', '기준', V1], ['2', '카운트 왼쪽', V2], ['3', '투구가 가운데', V3], ['4', '3등분', V4],
  ['5', '주루 크게', V5], ['6', '투구는 주루 아래', V6], ['7', '카운트 가로 두 줄', V7], ['8', '주루 왼쪽 끝', V8]];
  ...
  return (
    <div style={{ width: 1920, background: '#020409', minHeight: '100vh', padding: 20 }}>
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2 text-[13px] text-gray-400">
        <b className="text-[15px] text-white">아래 단 배치 · 8안</b>
        {V.map(([id, name]) => (
