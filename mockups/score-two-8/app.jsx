...
/* 2. 아래 단을 세 칸으로 — 카운트 · 주루 · 투구 수 */
const V2 = () => (
  <Frame board={(
    <Two w={258}>
      <div className="flex items-stretch"><InnCol /><Teams w={218} /></div>
      <div className="flex items-stretch border-t border-black/20">
        <span className="grid flex-1 place-items-center py-1"><Bso dot={13} /></span>
        <Sep />
        <span className="grid place-items-center px-2"><Diamond size={62} /></span>
        <Sep />
        <span className="grid place-items-center" style={{ width: 46 }}>
...
/* 3. 아래 단을 가로 한 줄로 — 가장 낮다 */
const V3 = () => (
  <Frame board={(
    <Two w={236}>
      <div className="flex items-stretch"><InnCol /><Teams w={196} /></div>
      <div className="flex items-center justify-between border-t border-black/20 px-3 py-1">
        <BsoRow dot={11} />
        <span className="flex items-center gap-2"><Diamond size={40} /><Pitch /></span>
      </div>
    </Two>
  )} />
);
...
/* 5. 아래 단을 한 톤 어둡게 — 두 단이 또렷하게 갈린다 */
const V5 = () => (
  <Frame board={(
    <Two>
      <div className="flex items-stretch"><InnCol /><Teams w={200} /></div>
      <div className="flex items-stretch" style={{ background: 'rgba(0,0,0,.08)' }}>
        <span className="grid flex-1 place-items-center py-1"><Bso dot={13} /></span>
        <Sep />
        <span className="grid place-items-center" style={{ width: 96 }}><Diamond size={66} note={G.pitches} /></span>
      </div>
...
/* 6. 회를 위 띠로 올리고 팀은 온전히 — 아래는 그대로 */
const V6 = () => (
  <Frame board={(
    <Two w={214}>
      <div className="flex items-center justify-between px-3 py-0.5" style={{ background: 'rgba(255,255,255,.5)' }}>
        <b className="font-display text-[14px] font-extrabold" style={{ color: INK }}><i className="mr-1 not-italic text-[#dc2626]">{G.top ? '▲' : '▼'}</i>{G.inning}</b>
      ...
      <div className="flex items-stretch border-t border-black/20">
        <span className="grid flex-1 place-items-center py-1"><Bso dot={13} /></span>
        <Sep />
        <span className="grid place-items-center" style={{ width: 88 }}><Diamond size={62} /></span>
      </div>
    </Two>
  )} />
);

/* 7. 아래 단을 넉넉하게 — 카운트와 주루를 크게 */
const V7 = () => (
  <Frame board={(
    <Two w={252}>
      <div className="flex items-stretch"><InnCol /><Teams w={212} /></div>
      <div className="flex items-stretch border-t border-black/20">
        <span className="grid flex-1 place-items-center py-2"><Bso dot={16} rowGap={6} font={15} /></span>
        <Sep />
        <span className="grid place-items-center" style={{ width: 104 }}><Diamond size={82} note={G.pitches} /></span>
      </div>
    </Two>
  )} />
);

/* 8. 두 단 사이를 선 대신 색 띠로 — 위아래가 한 몸처럼 이어진다 */
const V8 = () => (
  <Frame board={(
    <Two>
      <div className="flex items-stretch"><InnCol /><Teams w={200} /></div>
      <i className="block h-px" style={{ background: 'linear-gradient(90deg,rgba(0,0,0,.32),rgba(0,0,0,.08) 70%,transparent)' }} />
...
const V = [['1', '기준', V1], ['2', '세 칸', V2], ['3', '가로 한 줄', V3], ['4', '주루 왼쪽', V4],
  ['5', '아래 단 어둡게', V5], ['6', '회는 위 띠', V6], ['7', '아래 단 넉넉히', V7], ['8', '선 대신 색 띠', V8]];
  ...
  return (
    <div style={{ width: 1920, background: '#020409', minHeight: '100vh', padding: 20 }}>
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2 text-[13px] text-gray-400">
        <b className="text-[15px] text-white">두 단 스코어보드 · 8안</b>
        {V.map(([id, name]) => (
