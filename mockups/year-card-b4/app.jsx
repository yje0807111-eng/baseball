...
/* A. 큰 카드 아래 2열 격자 — 주인공 한 장, 나머지는 네모지게 깔린다 */
const VA = (y, set) => (
  <Shell y={y} set={set}>{(hero, rest) => (
    <div className="relative z-10 grid min-h-0 flex-1 gap-5" style={{ gridTemplateColumns: '344px minmax(0,1fr)' }}>
...
/* B. 큰 카드 옆 타일 벽 — 주인공 왼쪽, 나머지는 그 오른쪽에 3열로 */
const VB = (y, set) => (
  <Shell y={y} set={set}>{(hero, rest) => (
    <div className="relative z-10 flex min-h-0 flex-1 flex-col">
      <div className="mt-5 max-w-[560px]"><Title y={y} hero={hero} /></div>
      <div className="mt-auto grid items-end gap-3" style={{ gridTemplateColumns: '236px 300px minmax(0,1fr)' }}>
        <Ticket s={hero} w="100%" h={190} />
        <div className="grid gap-1.5" style={{ gridTemplateColumns: 'repeat(2,minmax(0,1fr))', gridAutoRows: '92px' }}>
          {rest.slice(0, 4).map((s) => <Ticket key={s.id} s={s} w="100%" h="100%" sm />)}
        </div>
        <Faces y={y} n={8} w={72} h={94} />
      </div>
    </div>
  )}</Shell>
);

/* C. 큰 카드는 왼쪽, 나머지는 바닥을 가로지른다 */
const VC = (y, set) => (
  <Shell y={y} set={set}>{(hero, rest) => (
    <div className="relative z-10 grid min-h-0 flex-1 gap-5" style={{ gridTemplateColumns: '300px minmax(0,1fr)' }}>
      <div className="flex min-h-0 flex-col justify-end pb-1">
        <Title y={y} hero={hero} big={false} />
        <div className="mt-4"><Ticket s={hero} w="100%" h={196} /></div>
      </div>
      <div className="flex min-h-0 flex-col justify-end pb-1">
        <Faces y={y} n={8} />
        <Lab t={`Series ${rest.length}`} />
...
/* D. 큰 카드 + 줄 목록 두 칸 — 이름과 인원이 또렷하게 */
const VD = (y, set) => (
  <Shell y={y} set={set}>{(hero, rest) => (
    <div className="relative z-10 flex min-h-0 flex-1 flex-col">
      <div className="mt-5 max-w-[520px]"><Title y={y} hero={hero} /></div>
      <div className="mt-auto grid items-end gap-4" style={{ gridTemplateColumns: '228px 392px minmax(0,1fr)' }}>
        <Ticket s={hero} w="100%" h={186} />
        <div className="grid gap-1.5" style={{ gridTemplateColumns: 'repeat(2,minmax(0,1fr))' }}>
          {rest.slice(0, 8).map((s) => (
            <span key={s.id} className="ui-cut flex items-center gap-2 px-2 py-1.5" style={{ '--c': '5px', background: 'rgba(255,255,255,.07)' }}>
              <span className="ui-cut block shrink-0 bg-[#0b1220] bg-cover" style={{ '--c': '4px', width: 26, height: 26, backgroundImage: art(topOf(s)), backgroundPosition: '60% 12%' }} />
              <b className="min-w-0 flex-1 truncate text-[12px] font-normal text-gray-200">{s.title}</b>
              {s.champion && trophy(12, 0.7)}
              <b className="font-display text-[11px] text-gray-500">{s.players.length}</b>
            </span>
          ))}
        </div>
        <Faces y={y} n={6} w={74} h={96} />
      </div>
    </div>
  )}</Shell>
);

const V = [['1', '아래 2열 격자', VA], ['2', '옆 타일 벽', VB], ['3', '바닥 가로줄', VC], ['4', '큰 카드 + 줄 목록', VD]];
  ...
  return (
    <div style={{ width: 1920, background: '#020409', minHeight: '100vh', padding: 20 }}>
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2 text-[13px] text-gray-400">
        <b className="text-[15px] text-white">큰 카드 + 나머지 · 4안</b>
        {V.map(([id, name]) => (
