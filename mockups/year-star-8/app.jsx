...
/* 1. 오른쪽 끝 2열 — 얼굴이 모서리에 붙고 가운데는 그림 */
const V1 = (y, set) => shell(y, set, '392px minmax(0,1fr) 230px', (yy) => (
  <>
    <span />
    <div className="flex min-h-0 flex-col justify-end pb-1">
      <Lab t="Stars" />
      <div className="mt-1.5 flex flex-wrap justify-end gap-1.5">{starsOf(yy, 8).map((p) => <Star key={p.id} p={p} w={72} h={94} />)}</div>
    </div>
  </>
));

/* 2. 오른쪽 바닥 한 줄 — 넓게 눕혀 그림 아래를 받친다 */
const V2 = (y, set) => shell(y, set, '392px minmax(0,1fr)', (yy) => (
  <div className="flex min-h-0 flex-col justify-end pb-1">
    <Lab t="Stars" />
    <div className="mt-1.5 flex gap-1.5">{starsOf(yy, 8).map((p) => <Star key={p.id} p={p} w={88} h={116} />)}</div>
  </div>
));

/* 3. 오른쪽 끝 한 줄 세로 — 가장 좁게, 그림을 가장 크게 */
const V3 = (y, set) => shell(y, set, '392px minmax(0,1fr) 104px', (yy) => (
  <>
    <span />
    <div className="flex min-h-0 flex-col justify-end gap-1.5 pb-1">
      <Lab t="Stars" />
      {starsOf(yy, 5).map((p) => <Star key={p.id} p={p} w="100%" h={98} />)}
    </div>
  </>
));

/* 4. 그 해 최고 한 명을 크게 · 나머지는 그 아래 작게 */
const V4 = (y, set) => shell(y, set, '392px minmax(0,1fr) 250px', (yy) => {
  ...
  return (
    <>
      ...
      <div className="flex min-h-0 flex-col justify-end pb-1">
        ...
        <div className="mt-1.5"><Star p={one} w="100%" h={250} /></div>
...
/* 5. 이름줄 — 얼굴을 작게 두고 이름 · 포지션 · 능력치를 읽게 */
const V5 = (y, set) => shell(y, set, '392px minmax(0,1fr) 262px', (yy) => (
  <>
    <span />
    <div className="flex min-h-0 flex-col justify-end pb-1">
      <Lab t="Stars" />
      <div className="mt-1.5 flex flex-col gap-1">
        {starsOf(yy, 8).map((p) => (
          <span key={p.id} className="ui-cut flex items-center gap-2 p-1" style={{ '--c': '6px', background: 'rgba(5,8,15,.66)' }}>
            <span className="ui-cut block shrink-0 bg-[#0b1220] bg-cover" style={{ '--c': '4px', width: 32, height: 32, backgroundImage: art(p), backgroundPosition: '60% 10%' }} />
...
/* 6. 계단 — 오른쪽 아래에서 위로 한 칸씩 어긋나게 */
const V6 = (y, set) => shell(y, set, '392px minmax(0,1fr) 300px', (yy) => (
  <>
    <span />
    <div className="flex min-h-0 flex-col justify-end pb-1">
      <Lab t="Stars" />
      <div className="mt-1.5 flex flex-col items-end gap-1.5">
        {starsOf(yy, 6).map((p, i) => (
          <span key={p.id} className="block" style={{ marginRight: i * 16 }}><Star p={p} w={92} h={70} /></span>
        ))}
      </div>
    </div>
  </>
...
/* 7. 오른쪽에도 판 하나 — 왼쪽과 짝이 되게 */
const V7 = (y, set) => shell(y, set, '392px minmax(0,1fr) 272px', (yy) => (
  <>
    <span />
    <div className="ui-cut ui-frame ui-glass mb-0 mt-auto flex flex-col p-3" style={{ '--c': '14px', '--a': A }}>
      <Lab t="Stars" />
      <div className="mt-1.5 grid gap-1.5" style={{ gridTemplateColumns: 'repeat(3,minmax(0,1fr))', gridAutoRows: '96px' }}>
        {starsOf(yy, 9).map((p) => <Star key={p.id} p={p} w="100%" h="100%" />)}
      </div>
    </div>
  </>
));

/* 8. 아주 조용하게 — 한 명만 카드로, 나머지는 이름 칩 */
const V8 = (y, set) => shell(y, set, '392px minmax(0,1fr) 230px', (yy) => {
  ...
  return (
    <>
      ...
      <div className="flex min-h-0 flex-col justify-end pb-1">
        <Lab t="Best of the year" />
        <div className="mt-1.5"><Star p={one} w="100%" h={230} /></div>
        <div className="mt-2 flex flex-wrap justify-end gap-1">
          {more.map((p) => (
            <span key={p.id} className="ui-cut px-2 py-1 text-[12px] text-gray-300" style={{ '--c': '5px', background: 'rgba(255,255,255,.07)' }}>
...
const V = [['1', '오른쪽 끝 2열', V1], ['2', '바닥 한 줄', V2], ['3', '끝 한 줄 세로', V3], ['4', '최고 한 명 크게', V4],
  ['5', '이름줄', V5], ['6', '계단', V6], ['7', '오른쪽도 판', V7], ['8', '한 명 + 이름 칩', V8]];
  ...
  return (
    <div style={{ width: 1920, background: '#020409', minHeight: '100vh', padding: 20 }}>
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2 text-[13px] text-gray-400">
        <b className="text-[15px] text-white">선수 구역 8안 (왼쪽 판 고정)</b>
        {V.map(([id, name]) => (
