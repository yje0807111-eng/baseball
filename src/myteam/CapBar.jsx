...
export default function CapBar({ team, sm = false, className = '' }) {
  const { cost, cap, left, over, quiet } = capUse(team);
  /* 캡의 90%(CAP_LOUD) 전에는 흐린 회색 — 이때는 골드가 막는다 */
  const c = over ? '#f87171' : left < cap * 0.04 ? '#fbbf24' : quiet ? '#6b7280' : '#10b981';
  ...
  return (
    <div className={`mt-cut ${sm ? 'px-3 py-1.5' : 'px-4 py-2.5'} ${className}`}
      ...
      <div className="flex items-baseline gap-2">
        <span className={`font-display font-bold tracking-[0.2em] text-gray-400 ${sm ? 'text-t4' : 'text-t4'}`}>CP</span>
        <b className={`font-display font-extrabold tabular-nums ${sm ? 'text-t3' : 'text-t2'}`} style={{ color: c }}>{cost.toLocaleString()}</b>
        <span className={`text-gray-400 ${sm ? 'text-t4' : 'text-t4'}`}>/ {cap.toLocaleString()}</span>
