              ...
              {STAFF_SLOTS.map((s) => {
                ...
                return (
                  <button key={s.key} type="button" onClick={() => { setStaffSlot(s.key); if (cur) setStaff(s.key, null); }}
                    ...
                    {cur && (
                      <span key={cur.id} className="mt-staff-in absolute inset-0 bg-cover bg-top transition-transform duration-300 group-hover:scale-105"
                        style={{ backgroundImage: `url(profiles/${encodeURIComponent(cur.id)}.webp), url(ui/mt/silhouette-coach.webp)` }} />
                    )}
