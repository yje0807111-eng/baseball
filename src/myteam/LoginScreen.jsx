          ...
          <div className="mt-cut mt-frame mt-8 bg-[#060a13]/88 p-7 backdrop-blur-[10px]" style={{ '--c': '16px' }}>
            ...
            <div className="mt-3 grid grid-cols-[1fr_auto] gap-3">
              <input value={nick} onChange={(e) => { setNick(e.target.value); setErr(''); }} onKeyDown={(e) => e.key === 'Enter' && go()}
                ...
                className="mt-cut mt-frame bg-white/[0.06] px-4 py-3.5 text-t2 text-white outline-none focus:shadow-[inset_0_0_0_2px_#10b981]" style={{ '--c': '10px' }} />
        ...
        <div className="mt-cut mt-frame absolute bottom-16 right-16 w-[330px] bg-[#060a13]/80 p-4 backdrop-blur-[10px]" style={{ '--c': '12px' }}>
          ...
          <div className="mt-3 flex items-center gap-3.5">
            ...
            <div className="min-w-0 flex-1">
              <b className="block truncate text-t2 text-white">{saved.nick}</b>
              <span className="text-t4 text-gray-400">{saved.team?.name || '나의 드림팀'} · {saved.team?.squad?.length || 0}/26 · {saved.team?.record?.w || 0}승 {saved.team?.record?.l || 0}패</span>
            </div>
