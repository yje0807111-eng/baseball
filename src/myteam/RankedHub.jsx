...
export function StandingsTable({ s, big = false, lastMoves = null }) {
  ...
  return (
    <table className={`w-full border-collapse text-right tabular-nums ${big ? 'text-[15px]' : 'text-[13px]'}`}>
      <thead>
        <tr className="font-display text-[11px] font-bold tracking-[0.14em] text-gray-500">
        ...
        {rows.map((r) => {
          ...
          return (
            <tr key={r.idx} className={`${cell} ${r.rank === POST_TEAMS ? 'border-b-2 border-dashed border-amber-300/50' : 'border-b border-white/[0.06]'}`}
              ...
              <td className={`${cell} text-center font-display text-lg font-extrabold`} style={{ color: r.rank <= POST_TEAMS ? '#fbbf24' : '#64748b' }}>{r.rank}</td>
              <td className={`${cell} max-w-0 pl-2 text-left`}>
                <span className="flex items-center gap-2">
                  <b className={`truncate ${mine ? 'text-[#34d399]' : 'text-white'}`}>{r.team.name}</b>
                  {move !== 0 && <em className="shrink-0 font-display text-xs not-italic" style={{ color: move > 0 ? ME : OPP }}>{move > 0 ? `▲${move}` : `▼${-move}`}</em>}
                </span>
              </td>
      ...
      {STAGES.map((st, i) => {
        ...
        const line = (idx, score, seedNo) => (
          <div className="flex items-baseline gap-2" style={{ opacity: res && res.winner !== idx ? 0.45 : 1 }}>
            <em className="w-7 shrink-0 font-display text-xs not-italic text-amber-300">{seedNo ? `${seedNo}위` : ''}</em>
            <b className="min-w-0 flex-1 truncate text-sm">{name(idx, seedNo)}</b>
            {score != null && <b className="font-display text-base text-white">{score}</b>}
          </div>
        );
        return (
          <div key={st.key} className="ui-cut flex flex-col gap-1 p-3"
            ...
            <p className="font-display text-[11px] font-bold tracking-[0.2em] text-amber-300">{st.en}</p>
            <b className="mb-auto text-lg font-black text-white">{st.ko}{i === 3 ? ' 🏆' : ''}</b>
      ...
      <header className="relative z-10 flex h-16 shrink-0 items-center gap-5 border-b px-6" style={{ borderColor: 'rgba(167,139,250,.3)', background: 'linear-gradient(180deg,rgba(5,8,15,.94),rgba(5,8,15,.6))' }}>
        ...
        <div className="ml-auto flex items-center gap-1.5">
          {Array.from({ length: GAMES }, (_, i) => {
            ...
            return (
              <span key={i} className="ui-cut grid h-8 w-8 place-items-center font-display text-sm font-bold"
          ...
          {STAGES.map((st, i) => {
            ...
            return (
              <span key={st.key} className="ui-cut px-2.5 py-1.5 font-display text-sm font-bold"
                  ...
                  {lastRound.map((g) => {
                    ...
                    const side = (idx, score, other) => (
                      <span className={`flex min-w-0 flex-1 items-baseline gap-2 ${idx === g.b ? 'flex-row-reverse text-right' : ''}`} style={{ opacity: score < other ? 0.55 : 1 }}>
                        <b className={`min-w-0 truncate text-sm ${idx === me ? 'text-[#34d399]' : 'text-white'}`}>{s.teams[idx].name}</b>
                        <b className="font-display text-xl text-white">{score}</b>
                      </span>
                    );
                    return (
                      <div key={`${g.a}-${g.b}`} className="ui-cut flex items-center gap-3 px-3 py-1.5" style={{ '--c': '7px', background: mineG ? 'rgba(52,211,153,.1)' : 'rgba(255,255,255,.03)' }}>
                        {side(g.a, g.as, g.bs)}<span className="font-display text-xs text-gray-500">:</span>{side(g.b, g.bs, g.as)}
                      </div>
                    );
                  })}
                </div>
        ...
        <aside className="ui-cut ui-frame ui-glass flex min-h-0 flex-col gap-3.5 p-6" style={{ '--c': '18px', '--a': oppTeam ? OPP : RK }}>
          {oppTeam ? (
            <>
              ...
              <button type="button" className="ui-btn ui-cut pri mt-auto min-h-[3.5rem] w-full text-lg" style={{ '--a': inPost ? '#fbbf24' : RK }} onClick={onPlay}>
            ...
            <>
              ...
              <div className="syn-scroll min-h-0 overflow-y-auto">
                {PLACE_REWARD.map((f, i) => {
                  ...
                  return (
                    <div key={f.ko} className="flex items-baseline justify-between border-b border-white/10 py-1.5 text-sm text-gray-300">
                      <span style={{ color: on ? '#fbbf24' : undefined }}>{i + 1}. {f.ko}</span>
                      <b className="font-display text-base" style={{ color: on ? '#fbbf24' : '#94a3b8' }}>{f.rp >= 0 ? '+' : ''}{f.rp} RP · {f.gold} G</b>
                    </div>
                  );
                })}
              </div>
